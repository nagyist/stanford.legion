...
namespace Realm {
  ...
  LocalNumaProcessor::~LocalNumaProcessor(void) { delete core_rsrv; }
  ...
  namespace Numa {
    ...
    /*static*/ Module *NumaModule::create_module(RuntimeImpl *runtime)
    {
      ...
      if(!numasysif_get_mem_info(meminfo) || !numasysif_get_cpu_info(cpuinfo)) {
      ...
      for(std::map<int, NumaNodeMemInfo>::const_iterator it = meminfo.begin();
        ...
        log_numa.info() << "NUMA memory node " << mi.node_id << ": "
                        << (mi.bytes_available >> 20) << " MB";
        ...
        if(mi.bytes_available >= mem_size) {
          ...
          // TODO: fatal error?
          log_numa.warning() << "insufficient memory in NUMA node " << mi.node_id << " ("
                             << mem_size << " > " << mi.bytes_available
                             << " bytes) - skipping allocation";
        }
      }
      for(std::map<int, NumaNodeCpuInfo>::const_iterator it = cpuinfo.begin();
          it != cpuinfo.end(); ++it) {
        const NumaNodeCpuInfo &ci = it->second;
        log_numa.info() << "NUMA cpu node " << ci.node_id << ": " << ci.cores_available
                        << " cores";
        if(ci.cores_available >= m->config->cfg_num_numa_cpus) {
          m->numa_cpu_counts[ci.node_id] = m->config->cfg_num_numa_cpus;
        } else {
          // TODO: fatal error?
          log_numa.warning() << "insufficient cores in NUMA node " << ci.node_id
                             << " - core assignment will fail";
          m->numa_cpu_counts[ci.node_id] = m->config->cfg_num_numa_cpus;
        }
      }
    ...
    void NumaModule::initialize(RuntimeImpl *runtime)
    {
      ...
      for(std::map<int, void *>::iterator it = numa_mem_bases.begin();
          it != numa_mem_bases.end(); ++it) {
        size_t mem_size = numa_mem_sizes[it->first];
        assert(mem_size > 0);
        void *base = numasysif_alloc_mem(it->first, mem_size, config->cfg_pin_memory);
        if(!base) {
          log_numa.fatal() << "allocation of " << mem_size << " bytes in NUMA node "
                           << it->first << " failed!";
          assert(false);
        }
        it->second = base;
    ...
    void NumaModule::cleanup(void)
    {
      ...
      for(std::map<int, void *>::iterator it = numa_mem_bases.begin();
        ...
        if(!ok)
          log_numa.error() << "failed to free memory in NUMA node " << it->first
                           << ": ptr=" << it->second;
      }
    }
