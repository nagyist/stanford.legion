                    ...
                    Context ctx, Runtime *runtime)
{
  ...
  BlasArrayRef<float> x = BlasArrayRef<float>::create(runtime, ctx, is_top, cs);
  BlasArrayRef<float> y = BlasArrayRef<float>::create(runtime, ctx, is_top, cs);

  x.fill(runtime, ctx, 2.0);
  y.fill(runtime, ctx, 3.0);

  axpy(runtime, ctx, 0.5f, x, y, cs);

  float result = dot(runtime, ctx, x, y);
