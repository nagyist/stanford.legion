                     ...
                     Context ctx, Runtime *runtime)
{
  ...
  initialize_gpu_array(ptr, rect.volume());
}
                ...
                Context ctx, Runtime *runtime)
{
  ...
  assert(acc_y.accessor.is_dense_arbitrary(rect));
  assert(acc_z.accessor.is_dense_arbitrary(rect));

  const double *x_ptr = acc_x.ptr(rect.lo);
  const double *y_ptr = acc_y.ptr(rect.lo);
  double *z_ptr = acc_z.ptr(rect.lo);

  gpu_saxpy(alpha, x_ptr, y_ptr, z_ptr, rect.volume());
}
