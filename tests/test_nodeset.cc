...
NodeID random_nodeid() { return lrand48() % num_nodes; }

class NodeSetWrapper {
public:
  NodeSetWrapper() { nodeset = new NodeSet; }

  ~NodeSetWrapper() { delete nodeset; }
  ...
  void validate() const
  {
    ...
    for(std::set<NodeID>::const_iterator it = expected.begin(); it != expected.end();
        ++it)
...
NodeSetWrapper *random_nodeset() { return &sets[lrand48() % num_nodesets]; }
...
int main(int argc, const char *argv[])
{
  ...
  NodeSetBitmask::configure_allocator(num_nodes - 1, 4, twolevel);
