...
pub fn deserialize<P: AsRef<Path>>(
    ...
    let file_name: String = path.as_ref().to_str().unwrap().to_string();
    let s = read_file(path)?;
    // throw error here if parse failed
    let (rest, records) = parse(&s, file_name, visible_nodes, filter_input).unwrap();
