           ...
           it != sources.end(); it++)
      {
        ...
        if (source_size != redop->sizeof_rhs)
        {
          ...
          error
              ...
              << ") does not have the right input size for the given reduction "
              << "operator. Future has size " << source_size
              << " bytes but reduction operator expects RHS inputs of "
              << redop->sizeof_rhs << " bytes.";
          error.raise();
        }
        LegionSpy::log_future_use(unique_op_id, impl->did);
      }
