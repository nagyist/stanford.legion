...
namespace Legion {
  namespace Internal {
      ...
      if (view->is_individual_view())
      {
        ...
        if (local_dids.size() > 1)
        {
          ...
          if (!!refinement_mask)
          {
            ...
            refinements.insert(
                static_cast<T*>(this)->clone((InstanceView*)nullptr/*no view*/, 
          ...
          if (!!refinement_mask)
          {
            ...
            refinements.insert(
                static_cast<T*>(this)->clone(
                  ...
                  collective_view : (InstanceView*)nullptr, refinement_mask, 
                  std::move(overlap)), refinement_mask);
            refinements.insert(
                static_cast<T*>(this)->clone((InstanceView*)nullptr/*view*/, 
    ...
    //--------------------------------------------------------------------------
    {
      ...
      {
        IndexSpaceExpression *valid_expr = fit->elements.empty() ? nullptr :
      ...
      if (!!uncovered)
      {
        ...
        if (fill_aggregator == nullptr)
        {
          ...
          fill_aggregator = new CopyFillAggregator(&analysis, 
              nullptr/*no previous guard*/, false/*track events*/);
    ...
    //--------------------------------------------------------------------------
    EquivalenceSet::EquivalenceSet(DistributedID id,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (replicate_logical_owner)
      {
        ...
        mapping->get_children(logical_owner_space, local_space,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      {
        for (FieldMaskSet<IndexSpaceExpression>::const_iterator it =
              tracing_dirty_fields->begin(); it !=
              tracing_dirty_fields->end(); it++)
          if (it->first->remove_nested_expression_reference(did))
            delete it->first;
    ...
    //--------------------------------------------------------------------------
    {
                    ...
                    restricted_instances.end(); rit++)
              {
                ...
                else if (rit->first == set_expr)
                {
                  ...
                  apply_restricted_reductions(rit->second, expr, expr_covers,
                      overlap, analysis.output_aggregator, nullptr/*no guard*/,
                ...
                else
                {
                  ...
                  if (expr_overlap->get_volume() == expr->get_volume())
                    ...
                    apply_restricted_reductions(rit->second, expr_overlap,
                        ...
                        analysis.trace_info, nullptr/*no applied expr tracking*/);
                }
              }
            }
          }
                ...
                !(user_mask * read_only_guards.get_valid_mask()))
      {
        ...
        if (!!user_mask)
        {
          ...
          update_set_internal(input_aggregator, nullptr/*no previous guard*/,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      Serializer rez;
      {
        RezCheck z(rez);
        rez.serialize(did);
        pack_state(rez, new_logical_owner, did, set_expr, set_expr,
    ...
    //--------------------------------------------------------------------------
    {
      RtUserEvent to_trigger;
      {
        ...
        for (std::vector<AddressSpaceID>::iterator it =
              ...
              replicated_owner_state->children.end(); /*nothing*/)
        {
          if ((*it) != owner)
          {
            Serializer rez;
    ...
    //--------------------------------------------------------------------------
    bool EquivalenceSet::filter_partial_invalidations(const FieldMask &mask,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!!reduce_mask)
      {
        ...
        apply_reductions(target_instances, target_views, expr, expr_covers,
                         ...
                         is_write ? nullptr : &applied_reductions);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!partial_instances.empty())
      {
        ...
        aggregator->record_partial_updates(target, target_manager, 
                                 ...
                                 trace_info, trace_info.recording ? this : nullptr,
                                 redop, across_helper);
      }
    }
                    ...
                    if (!overlap->is_empty())
                    {
                      ...
                      if ((fill_expr == expr) && masked_exprs.empty())
                      {
                        ...
                        IndexSpaceExpression *merged_expr =
                          ...
                          fill_expr : runtime->union_index_spaces(
                              fill_expr, it->second);
                      ...
                      fill_expr = runtime->subtract_index_spaces(
                          fill_expr, overlap);
#ifdef DEBUG_LEGION
    ...
    //--------------------------------------------------------------------------
    {
            ...
            reduction_targets.begin(); it != reduction_targets.end(); it++)
      {
        ...
        if (!inst_mask)
          continue;
        apply_reduction(it->first, nullptr/*no manager since this is restricted*/,
                        expr, expr_covers, inst_mask, aggregator,
                        previous_guard, analysis, track_events,
                        trace_info, applied_exprs, nullptr/*across*/);
      }
    }
      ...
      while (fidx >= 0)
      {
        ...
        if (expr_covers)
        {
          ...
          aggregator->record_reductions(target, target_manager, 
              finder->second, fidx, (across_helper == nullptr) ? fidx : 
        ...
        else
        {
          ...
          if (!to_record.empty())
          {
            ...
            aggregator->record_reductions(target, target_manager, to_record,
                                    fidx, (across_helper == nullptr) ? fidx : 
                                      across_helper->convert_src_to_dst(fidx),
    ...
    //--------------------------------------------------------------------------
    {
            ...
            rit != restricted_instances.end(); rit++)
      {
              ...
              it != restricted_copies.end(); it++)
        {
          ...
          if (aggregator == nullptr)
            aggregator = new CopyFillAggregator(analysis,
                                nullptr/*no previous guard*/, false/*track*/);
          aggregator->record_update(it->first.first, nullptr/*no manager*/,
      ...
      if (analysis.target_views.empty())
      {
              ...
              to_update.begin(); it != to_update.end(); it++)
        {
                ...
                it->second.begin(); vit != it->second.end(); vit++, index++)
          {
            ...
            update_set_internal(analysis.release_aggregator, nullptr/*no guard*/,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!analysis.perfect)
      {
        ...
        {
          make_instances_valid(across_aggregator, nullptr/*no guard*/,
              ...
              vit != partial_valid_instances.end(); vit++)
        {
          ...
          else
          {
                  ...
                  vit->second.begin(); it != vit->second.end(); it++)
            {
              ...
              if (aggregator == nullptr)
                aggregator = new CopyFillAggregator(analysis,
                        nullptr/*no previous guard*/, false/*track*/, true_guard);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else
      {
        target->receive_trace_conditions(tracing_preconditions,
            ...
            unique_dirty_exprs, parent_req_index, tree_id, result);
        if (ready_event.exists())
    ...
    //--------------------------------------------------------------------------
    {
      ...
      void *location = runtime->find_or_create_pending_collectable_location<
                                                        EquivalenceSet>(did);
      EquivalenceSet *set = new(location) EquivalenceSet(did, 
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else
      {
        ...
        src->clone_to_remote(did, logical_owner, set_expr, clone_expr,
             mask, applied_events, record_invalidate);
      }
    }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      find_overlap_updates(expr, expr_covers, mask, pack_invalidates,
                           ...
                           pack_guards ? &reduction_fill_guards : nullptr, 
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if ((read_only_updates != nullptr) && !read_only_updates->empty())
      {
        ...
        rez.serialize<size_t>(0);
      if ((reduction_fill_updates != nullptr) && !reduction_fill_updates->empty())
      {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (num_read_only_guards)
      {
        ...
        {
          CopyFillGuard *guard = 
            CopyFillGuard::unpack_guard(derez, this);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!read_only_guards.empty() && 
          (read_only_guard_updates != nullptr) &&
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (target_space == runtime->address_space)
      {
        ...
        set->clone_to_remote(target, target_space, target_expr, overlap, mask,
            applied_events, record_invalidate);
      if (!applied_events.empty())
    ...
    EqSetTracker::EqSetTracker(LocalLock &lock)
      : tracker_lock(lock), pending_equivalence_sets(nullptr),
        ...
        equivalence_sets_ready(nullptr), waiting_infos(nullptr),
        creation_requests(nullptr), creation_rectangles(nullptr),
        creation_sources(nullptr), remaining_responses(nullptr)
    //--------------------------------------------------------------------------
    {
    }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      assert(this == targets[
          target_mapping.find_index(runtime->address_space)]);
    ...
    //--------------------------------------------------------------------------
    {
            ...
            rectangle_sets.begin(); rit != rectangle_sets.end(); rit++)
      {
        ...
        if (!(rit->set_mask * unique_sources.get_valid_mask()) &&
            check_for_congruent_source_equivalence_sets(*rit,
              ready_events, created_sets, unique_sources,
      ...
      {
        for (FieldMaskSet<EquivalenceSet>::const_iterator it =
              created_sets.begin(); it != created_sets.end(); it++)
        {
              ...
              eit->second.begin(); sit != eit->second.end(); /*nothing*/)
        {
          ...
          if (eit->first != target)
          {
            ...
            target->clone_from(eit->first, overlap, expression,
                record_invalidate, ready_events);
          }
          sit->set_mask -= overlap;
          if (!sit->set_mask)
          {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      void *location = 
        runtime->find_or_create_pending_collectable_location<EquivalenceSet>(
          did);
      EquivalenceSet *set = new(location) EquivalenceSet(did, root, 
        ...
        else
        {
                ...
                sit->second.begin(); it != sit->second.end(); it++)
          {
            references_to_remove += it->first->invalidate_equivalence_sets(
                it->second, owner, runtime->address_space, 
                invalidated_events);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      fargs->tracker->finalize_equivalence_sets(fargs->compute,
          fargs->enclosing, fargs->outermost, fargs->parent_req_index,
