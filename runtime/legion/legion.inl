...
#undef PHYSICAL_REGION_CONSTRUCTORS_WITH_BOUNDS
    ...
    //--------------------------------------------------------------------------
    {
      ...
      result.instance = instance;
      if (!Realm::AffineAccessor<
          typename REDOP::RHS,1,coord_t>::is_compatible(instance, 0)) 
        UntypedDeferredValue::report_incompatible_accessor("AffineAccessor");
      // We can make the accessor
      result.accessor =
...
#undef DIMFUNC
    ...
    //--------------------------------------------------------------------------
    {
      return IndexPartitionT<DIM,T>(create_restricted_partition(ctx,
        parent, color_space, &transform, sizeof(transform), &extent, 
    ...
    //--------------------------------------------------------------------------
    {
      ...
      return IndexPartitionT<DIM,T>(create_restricted_partition(ctx,
        parent, color_space, &transform, sizeof(transform), &extent, 
        sizeof(extent), LEGION_DISJOINT_KIND, color, 
        provenance, __func__, true));
    }
