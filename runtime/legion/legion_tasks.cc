...
namespace Legion {
  namespace Internal {
    ...
    //--------------------------------------------------------------------------
    ExternalTask::ExternalTask(void) : Task(), arg_manager(NULL)
    //--------------------------------------------------------------------------
    { }
    ...
    //--------------------------------------------------------------------------
    void ExternalTask::unpack_external_task(
    ...
    //--------------------------------------------------------------------------
    /*static*/ void TaskOp::process_unpack_task(
      ...
      for (unsigned idx = 0; idx < logical_regions.size(); idx++)
      {
        ...
        if (req.privilege != LEGION_NO_ACCESS && req.privilege_fields.empty())
          REPORT_LEGION_WARNING(
              LEGION_WARNING_REGION_REQUIREMENT_TASK,
        ...
        if (IS_READ_ONLY(req) && ((req.privilege & LEGION_DISCARD_INPUT_MASK) ==
          ...
          REPORT_LEGION_ERROR(
              ERROR_INVALID_DISCARD_QUALIFIER,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (local_function && !runtime->is_local(target_proc))
        REPORT_LEGION_ERROR(
            ...
            "task %s (UID %lld) be assigned to processor " IDFMT
            " which is "
            "not local to address space %d. Local function tasks must be "
            "assigned to local processors.",
            mapper->get_mapper_name(), get_task_name(), get_unique_id(),
      ...
      if (parent_priority != options.parent_priority)
      {
        ...
        if (parent_ctx->is_priority_mutable())
          ...
          REPORT_LEGION_WARNING(
              LEGION_WARNING_INVALID_PRIORITY_CHANGE,
              ...
              parent_ctx->get_unique_id(), get_task_name(), get_unique_id())
      }
      if (is_index_space)
      {
        if (!options.check_collective_regions.empty())
        {
               ...
               it != options.check_collective_regions.end(); it++)
          {
            ...
            if (!IS_WRITE(req))
              ...
              REPORT_LEGION_WARNING(
                  LEGION_WARNING_WRITE_PRIVILEGE_COLLECTIVE,
                  ...
                  mapper->get_mapper_name(), *it, get_task_name(), unique_op_id)
          }
        }
        for (unsigned idx = 0; idx < regions.size(); idx++)
      ...
      if (options.replicate)
      {
        ...
        {
          REPORT_LEGION_WARNING(
              LEGION_WARNING_UNSUPPORTED_REPLICATION,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < regions.size(); idx++)
      {
        ...
        int bad_index = -1;
        LegionErrorType et =
            runtime->verify_requirement(regions[idx], bad_field);
        ...
        switch (et)
        {
          ...
          case ERROR_INVALID_REGION_HANDLE:
            {
              REPORT_LEGION_ERROR(
                  ...
                  regions[idx].region.tree_id, idx, get_task_name(),
                  get_unique_id())
              break;
            }
          case ERROR_INVALID_PARTITION_HANDLE:
            {
              REPORT_LEGION_ERROR(
                  ERROR_INVALID_PARTITION_HANDLE,
                  ...
                  regions[idx].partition.tree_id, idx, get_task_name(),
                  get_unique_id())
              break;
            }
          case ERROR_BAD_PROJECTION_USE:
            {
              REPORT_LEGION_ERROR(
                  ERROR_PROJECTION_REGION_REQUIREMENT,
          ...
          case ERROR_NON_DISJOINT_PARTITION:
            {
              REPORT_LEGION_ERROR(
                  ERROR_NONDISJOINT_PARTITION_SELECTED,
          ...
          case ERROR_FIELD_SPACE_FIELD_MISMATCH:
            {
              ...
              REPORT_LEGION_ERROR(
                  ...
                  "(ID %lld)",
                  bad_field, sp.id, idx, get_task_name(), get_unique_id())
              break;
            }
          case ERROR_INVALID_INSTANCE_FIELD:
            {
              REPORT_LEGION_ERROR(
                  ERROR_INSTANCE_FIELD_PRIVILEGE,
                  "Instance field %d is not one of the "
                  "privilege fields for region %d of "
                  "task %s (ID %lld)",
                  bad_field, idx, get_task_name(), get_unique_id())
              break;
            }
          case ERROR_DUPLICATE_INSTANCE_FIELD:
            {
              REPORT_LEGION_ERROR(
                  ERROR_INSTANCE_FIELD_DUPLICATE,
          ...
          case ERROR_BAD_PARENT_REGION:
            {
              if (bad_index < 0)
                ...
                REPORT_LEGION_ERROR(
                    ...
                    regions[idx].parent.index_space.id,
                    regions[idx].parent.field_space.id,
                    regions[idx].parent.tree_id, idx, bad_index, bad_field)
              break;
            }
          case ERROR_BAD_REGION_PATH:
            {
              REPORT_LEGION_ERROR(
                  ERROR_REGION_NOT_SUBREGION,
                  ...
                  regions[idx].region.index_space.id,
                  regions[idx].region.field_space.id,
                  regions[idx].region.tree_id, PRINT_REG(regions[idx].parent),
                  idx, get_task_name(), get_unique_id())
              break;
            }
          case ERROR_BAD_PARTITION_PATH:
            {
              REPORT_LEGION_ERROR(
                  ERROR_PARTITION_NOT_SUBPARTITION,
                  ...
                  PRINT_REG(regions[idx].parent), idx, get_task_name(),
                  get_unique_id())
              break;
            }
          ...
          case ERROR_BAD_PARTITION_PRIVILEGES:
            {
              REPORT_LEGION_ERROR(
                  ERROR_PRIVILEGES_PARTITION_NOTSUBSET,
           ...
           tree_it != tree_indexes.end(); tree_it++)
      {
        ...
        std::vector<IndexTreeNode*> index_nodes(indexes.size());
        {
          FieldSpaceNode* field_space_node =
              runtime->forest->get_node(logical_regions[indexes[0]].parent)
                  ->column_source;
          for (unsigned idx = 0; idx < indexes.size(); idx++)
          {
        ...
        for (unsigned i = 1; i < indexes.size(); i++)
        {
          ...
          for (unsigned j = 0; j < i; j++)
          {
            ...
            if (runtime->forest->are_disjoint_tree_only(
                    index_nodes[i], index_nodes[j], common_ancestor))
              continue;
#ifdef DEBUG_LEGION
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (impl->is_concurrent() && !concurrent_task && !must_epoch_task &&
        ...
        REPORT_LEGION_ERROR(
            ...
            local_mapper->get_mapper_name(), get_task_name(), get_unique_id(),
            impl->get_name())
      else if (
          concurrent_task && !must_epoch_task && !impl->is_concurrent() &&
          is_index_space && (index_domain.get_volume() > 1))
        REPORT_LEGION_WARNING(
            LEGION_WARNING_UNUSED_CONCURRENCY,
            ...
            local_mapper->get_mapper_name(), impl->get_name(), get_task_name(),
            get_unique_id())
...
#undef CONSTRAINT_NAMES
          ...
          REPORT_LEGION_ERROR(
              ...
              get_unique_id(), it->first, constraint_name)
        }
      }
      ...
      if (execution_constraints.processor_constraint.is_valid())
      {
        ...
        if (!execution_constraints.processor_constraint.can_use(kind))
        {
          ...
          REPORT_LEGION_ERROR(
              ...
              local_mapper->get_mapper_name(), impl->vid, get_task_name(),
              get_unique_id(), Processor::get_kind_name(kind))
        }
      }
             ...
             iit != con_it->indexes.end(); iit++, idx++)
        {
            ...
            for (unsigned idx = 0; idx < insts.size(); idx++)
            {
              ...
              if (man->is_virtual_manager())
                REPORT_LEGION_ERROR(
                    ...
                    local_mapper->get_mapper_name(), *iit, get_task_name(),
                    get_unique_id(), impl->vid)
          ...
          else
          {
            ...
            if (req.region.get_tree_id() != tree_id)
              REPORT_LEGION_ERROR(
                  ERROR_INVALID_LOCATION_CONSTRAINT,
                  ...
                  *(con_it->indexes.begin()), *iit, impl->vid, get_task_name(),
                  tree_id, req.region.get_tree_id())
            const InstanceSet& insts = physical_instances[*iit];
            if (local_mapper == NULL)
              local_mapper = runtime->find_mapper(current_proc, map_id);
                ...
                if (finder != colocation_instances.end())
                {
                  if (finder->second.first->get_instance() !=
                    ...
                    REPORT_LEGION_ERROR(
                        ...
                        "Requirement %d mapped to instance " IDFMT
                        " while "
                        ...
                        *iit, manager->get_instance().id, finder->second.second,
      ...
      for (unsigned idx = 0; idx < logical_regions.size(); idx++)
      {
        ...
        if (parent_index < 0)
          REPORT_LEGION_ERROR(
              ...
              use_ctx->get_task_name(), use_ctx->get_unique_id(),
              get_task_name(), get_unique_id(),
    ...
    //--------------------------------------------------------------------------
    {
      ...
      LegionSpy::log_logical_requirement(
          uid, idx, reg,
          reg ? req.region.index_space.id : req.partition.index_partition.id,
          reg ? req.region.field_space.id : req.partition.field_space.id,
          reg ? req.region.tree_id : req.partition.tree_id, req.privilege,
          req.prop, req.redop, req.parent.index_space.id);
    ...
    //--------------------------------------------------------------------------
    SingleTask::SingleTask(Runtime* rt) : TaskOp(rt)
    //--------------------------------------------------------------------------
    { }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (map_origin)
      {
        ...
        for (unsigned idx = 0; idx < logical_regions.size(); idx++)
        {
          ...
          if (result)
            version_infos[idx].unpack_equivalence_sets(
        ...
        for (unsigned idx = 0; idx < num_phy; idx++)
          physical_instances[idx].unpack_references(
              runtime, derez, ready_events);
        size_t num_pre;
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < logical_regions.size(); idx++)
      {
        ...
        if ((regions.size() <= idx) && !is_output_valid(idx - regions.size()))
        {
          ...
          runtime->forest->perform_versioning_analysis(
              this, idx, req, version_info, ready_events, &output_ready);
#ifdef DEBUG_LEGION
          ...
          runtime->forest->perform_versioning_analysis(
              this, idx, req, version_info, ready_events,
              NULL /*output region*/,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < regions.size(); idx++)
      {
            ...
            (regions[idx].privilege != LEGION_REDUCE))
        {
          ...
          runtime->forest->physical_premap_region(
              this, idx, regions[idx], version_infos[idx], current_valid,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (is_remote() && is_recording() && (remote_trace_recorder == NULL))
      {
        remote_trace_recorder = new RemoteTraceRecorder(
            runtime, orig_proc.address_space(), get_trace_local_id(), tpl,
            0 /*did*/, 0 /*tid*/);
      ...
      if (must_epoch_owner == NULL)
      {
        ...
        {
          REPORT_LEGION_WARNING(
              LEGION_WARNING_EMPTY_OUTPUT_TARGET,
      ...
      else
      {
        ...
        {
          REPORT_LEGION_WARNING(
              LEGION_WARNING_IGNORING_SPURIOUS_TARGET,
        ...
        {
          REPORT_LEGION_WARNING(
              LEGION_WARNING_IGNORING_PROCESSOR_REQUEST,
              "Ignoring processor request of " IDFMT
              " for "
              "task %s (ID %lld) by mapper %s because task "
              "has already been mapped to processor " IDFMT
              " as part of a must epoch launch.",
              output.target_procs[0].id, get_task_name(), get_unique_id(),
              mapper->get_mapper_name(), this->target_proc.id);
        }
      ...
      if (!futures.empty())
      {
        ...
        for (unsigned idx = future_memories.size(); idx < futures.size(); idx++)
        {
          ...
          // Safe to block here indefinitely waiting for unbounded pools
          futures[idx].impl->request_application_instance(
              target_memory, this, NULL /*safe_for_unbounded_pools*/);
        }
      }
      ...
      if (!output.task_prof_requests.empty())
      {
        ...
        for (std::set<ProfilingMeasurementID>::const_iterator it =
                 output.task_prof_requests.requested_measurements.begin();
             it != output.task_prof_requests.requested_measurements.end(); it++)
        {
          ...
          switch ((Realm::ProfilingMeasurementID)*it)
          {
            ...
            default:
              {
                REPORT_LEGION_WARNING(
                    LEGION_WARNING_MAPPER_REQUESTED_PROFILING,
      ...
      if (!runtime->unsafe_mapper)
      {
        if (target_processors.size() > 1)
        {
          ...
          for (std::vector<Processor>::const_iterator it =
                   target_processors.begin();
               it != target_processors.end(); it++)
      ...
      for (unsigned idx = 0; idx < regions.size(); idx++)
      {
        ...
        if (!output.source_instances[idx].empty())
          runtime->forest->physical_convert_sources(
              this, regions[idx], output.source_instances[idx],
              source_instances[idx], acquired);
        int composite_idx = runtime->forest->physical_convert_mapping(
            this, regions[idx], output.chosen_instances[idx], result, bad_tree,
            missing_fields, acquired, unacquired, !runtime->unsafe_mapper);
        if (bad_tree > 0)
          REPORT_LEGION_ERROR(
              ERROR_INVALID_MAPPER_OUTPUT,
              "Invalid mapper output from invocation of '%s' on "
              ...
              "map_task", mapper->get_mapper_name(), bad_tree, idx,
              get_task_name(), get_unique_id(),
        ...
        if (!missing_fields.empty())
        {
          ...
          REPORT_LEGION_ERROR(
              ...
              "map_task", mapper->get_mapper_name(), missing_fields.size(), idx,
              get_task_name(), get_unique_id())
        }
        if (!unacquired.empty())
        {
        ...
        if (composite_idx >= 0)
        {
          ...
          if (result.size() > 1)
            REPORT_LEGION_ERROR(
                ...
                "map_task", mapper->get_mapper_name(), idx, get_task_name(),
                get_unique_id())
            ...
            for (unsigned idx2 = 0; idx2 < result.size(); idx2++)
            {
              ...
              if (visible_memories.find(mem) == visible_memories.end())
                ...
                REPORT_LEGION_ERROR(
                    ...
                    "for task %s (ID %lld).",
                    "map_task", mapper->get_mapper_name(), idx, mem.id,
                    get_task_name(), get_unique_id())
            }
          }
            ...
            for (unsigned idx2 = 0; idx2 < result.size(); idx2++)
            {
              ...
              if (!manager->is_reduction_manager())
                ...
                REPORT_LEGION_ERROR(
                    ...
                    "map_task", mapper->get_mapper_name(), manager->redop, idx,
                    get_task_name(), get_unique_id(), regions[idx].redop)
            }
          }
          else
          {
            for (unsigned idx2 = 0; idx2 < result.size(); idx2++)
              if (result[idx2].get_manager()->is_reduction_manager())
                REPORT_LEGION_ERROR(
                    ...
                    "does not have reduction privileges.",
                    "map_task", mapper->get_mapper_name(), idx, get_task_name(),
                    get_unique_id())
          }
        }
          ...
          for (unsigned idx = 0; idx < output_regions.size(); idx++)
          {
            ...
            if (!target.exists() ||
              ...
              REPORT_LEGION_ERROR(
                  ...
                  "map_task", mapper->get_mapper_name(), target.id, idx,
                  get_task_name(), get_unique_id())
          }
      ...
      {
        for (std::set<unsigned>::const_iterator it =
                 output.untracked_valid_regions.begin();
             it != output.untracked_valid_regions.end(); it++)
        {
          ...
          if ((*it >= regions.size()) || !IS_READ_ONLY(regions[*it]))
          {
            if (*it < regions.size())
              REPORT_LEGION_WARNING(
                  LEGION_WARNING_NON_READ_ONLY_UNTRACK_VALID,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      {
        REPORT_LEGION_ERROR(
            ERROR_INVALID_OUTPUT_REGION_CONSTRAINTS,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (configuration.auto_tracing_enabled)
      {
        ...
        inner_ctx = new AutoTracing<InnerContext>(
            configuration, runtime, this, get_depth(), false /*is inner*/,
            regions, output_regions, parent_req_indexes, virtual_mapped,
            task_priority, ApEvent::NO_AP_EVENT, 0 /*did*/, false /*inline*/,
            true /*implicit*/);
      }
      else
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (context_configuration.hysteresis_percentage > 100)
        REPORT_LEGION_ERROR(
            ...
            context_configuration.hysteresis_percentage, get_task_name(),
            get_unique_id())
      ...
      {
        log_auto_trace.warning(
            "Waring disabling automatic tracing requested "
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < processors.size(); idx++)
      {
        ...
        if (!proc.exists())
          ...
          REPORT_LEGION_ERROR(
              ERROR_INVALID_MAPPER_OUTPUT,
              "Invalid mapper output. Mapper %s requested processor " IDFMT
              " which is of kind %s when mapping task %s "
              "(ID %lld), but the target processor " IDFMT
              " has "
        ...
        if (proc.address_space() != space)
          REPORT_LEGION_ERROR(
              ERROR_INVALID_MAPPER_OUTPUT,
              "Invalid mapper output. Mapper %s requested processor " IDFMT
              " which is in address space %d when mapping "
              "task %s (ID %lld) but the target processor " IDFMT
              "is in address space %d. All target processors must "
              "be in the same address space.",
              mapper->get_mapper_name(), proc.id, proc.address_space(),
              get_task_name(), get_unique_id(), this->target_proc.id, space)
      }
    }
          ...
          if (finder == output.leaf_pool_bounds.end())
            finder =
                output.leaf_pool_bounds
                    .insert(std::make_pair(it->first, it->second->get_bounds()))
                    .first;
    ...
    //--------------------------------------------------------------------------
    {
      ...
      {
        REPORT_LEGION_WARNING(
            LEGION_WARNING_IGNORED_REPLICATION,
      ...
      else
      {
        ...
        if (output.leaf_variants.size() != output.target_processors.size())
          REPORT_LEGION_ERROR(
              ...
              "replicate_task", mapper->get_mapper_name(),
              output.leaf_variants.size(), output.target_processors.size(),
              get_task_name(), get_unique_id())
        for (unsigned idx = 0; idx < output.leaf_variants.size(); idx++)
        {
          ...
          if (impl == NULL)
            REPORT_LEGION_ERROR(
                ...
                "to be replicated.",
                "replicate_task", mapper->get_mapper_name(),
                output.leaf_variants[idx], get_task_name(), get_unique_id())
          if (var_impl == NULL)
            var_impl = impl;
            ...
            for (unsigned idx = 0; idx < output.target_processors.size(); idx++)
              if (!constraint.can_use(output.target_processors[idx].kind()))
                REPORT_LEGION_ERROR(
                    ...
                    output.target_processors[idx].id, output.chosen_variant,
                    get_task_name(), get_unique_id())
          }
        }
        else
...
#undef PROC_NAMES
          ...
          for (unsigned idx = 0; idx < output.target_processors.size(); idx++)
          {
            ...
            if (constraint.is_valid() &&
              ...
              REPORT_LEGION_ERROR(
                  ...
                  "replicate_task", mapper->get_mapper_name(),
                  proc_names[output.target_processors[idx].kind()],
                  output.target_processors[idx].id, output.leaf_variants[idx],
                  get_task_name(), get_unique_id())
          }
        }
        ...
        if (!output.shard_points.empty())
        {
          ...
          if (output.shard_points.size() != output.target_processors.size())
            REPORT_LEGION_ERROR(
                ...
                "replicate_task", mapper->get_mapper_name(),
                output.shard_points.size(), output.target_processors.size(),
                get_task_name(), get_unique_id())
        ...
        if (output.shard_domain.exists())
        {
          ...
          if (output.shard_points.size() != output.shard_domain.get_volume())
            REPORT_LEGION_ERROR(
                ...
                "replicate_task", mapper->get_mapper_name(),
                output.shard_points.size(), output.shard_domain.get_volume(),
                get_task_name(), get_unique_id())
      ...
      if (!output.shard_points.empty())
      {
        ...
        for (unsigned idx = 0; idx < output.shard_points.size(); idx++)
        {
          ...
          if (output.shard_points[idx].get_dim() != dim)
            REPORT_LEGION_ERROR(
                ...
                mapper->get_mapper_name(), dim,
                output.shard_points[idx].get_dim(), get_task_name(),
                get_unique_id())
        ...
        if ((domain_dim > 0) && (domain_dim != dim))
          REPORT_LEGION_ERROR(
              ...
              mapper->get_mapper_name(), domain_dim, dim, get_task_name(),
              get_unique_id())
      }
      else
      {
      ...
      shard_manager = new ShardManager(
          runtime, manager_did, mapping, local_shards.size(), configuration,
      ...
      if (region_count > 0)
      {
          ...
          if (!no_access_regions[0] && !virtual_mapped[0])
          {
            ...
            region_preconditions.back() =
                runtime->forest->physical_perform_updates_and_registration(
                    regions[0], version_infos[0], this, 0, init_precondition,
        ...
        else
        {
          ...
          if (read_only_count > 1)
          {
            ...
            for (std::vector<unsigned>::const_iterator it =
                     performed_regions.begin();
                 it != performed_regions.end(); it++)
            {
            ...
            if (!read_only_preconditions.empty())
            {
              ...
              {
                for (std::vector<unsigned>::const_iterator it =
                         read_only_regions.begin();
                     it != read_only_regions.end(); it++)
               ...
               it != performed_regions.end(); it++)
          {
            region_preconditions[*it] =
                runtime->forest->physical_perform_registration(
                    reg_pre[*it], analyses[*it], map_applied_conditions,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < regions.size(); idx++)
      {
        ...
        if (request_valid_instances)
        {
          ...
          FieldMaskSet<ReplicatedView> collectives;
          runtime->forest->physical_premap_region(
              this, idx, regions[idx], get_version_info(idx), postmap_valid,
              collectives, map_applied_conditions);
      ...
      for (unsigned idx = 0; idx < regions.size(); idx++)
      {
        ...
        {
          REPORT_LEGION_WARNING(
              LEGION_WARNING_MAPPER_REQUESTED_POST,
        ...
        RegionTreeID bad_tree = 0;
        std::vector<PhysicalManager*> unacquired;
        bool had_composite = runtime->forest->physical_convert_postmapping(
            this, req, output.chosen_instances[idx], result, bad_tree,
        ...
        if (bad_tree > 0)
          REPORT_LEGION_ERROR(
              ...
              "%d.",
              mapper->get_mapper_name(), bad_tree, idx, get_task_name(),
              get_unique_id(), regions[idx].region.get_tree_id())
        if (!unacquired.empty())
        {
          std::map<PhysicalManager*, unsigned>* acquired_instances =
              get_acquired_instances_ref();
        ...
        {
          REPORT_LEGION_WARNING(
              LEGION_WARNING_MAPPER_REQUESTED_COMPOSITE,
        ...
        if (!output.source_instances[idx].empty())
          runtime->forest->physical_convert_sources(
              this, regions[idx], output.source_instances[idx], sources,
      ...
      if (future_return_size && (*future_return_size < instance->size))
      {
        ...
        if (provenance != NULL)
          ...
          REPORT_LEGION_ERROR(
              ...
              get_task_name(), get_unique_id(), *future_return_size,
              instance->size)
      }
    }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (std::map<Memory::Kind, PoolBounds>::const_iterator it =
               variant->leaf_pool_bounds.begin();
           it != variant->leaf_pool_bounds.end(); it++)
      {
...
#undef MEM_NAMES
          ...
          REPORT_LEGION_ERROR(
              ...
              "from static constraint.",
              mem_names[it->first], target_proc.id, get_task_name(),
              get_unique_id());
        }
            ...
            if (finder->second.is_bounded())
            {
              ...
              if (static_bounds.size < dynamic_bounds.size)
                REPORT_LEGION_ERROR(
                    ...
                    "task variant.",
                    mapper->get_mapper_name(), dynamic_bounds.size,
                    manager->get_name(), get_task_name(), get_unique_id(),
                    variant->vid, static_bounds.size)
              ...
              REPORT_LEGION_ERROR(
                  ...
                  get_task_name(), get_unique_id(), variant->vid,
                  it->second.size);
          }
...
#undef MEM_NAMES
              ...
              REPORT_LEGION_WARNING(
                  LEGION_WARNING_UNBOUND_MEMORY_POOL,
                  ...
                  variant->get_name(), get_task_name(), get_unique_id(),
                  mem_names[it->first], mapper->get_mapper_name())
            }
          }
        }
        else
...
#undef MEM_NAMES
          ...
          !check_collective_regions.empty())
      {
        ...
        for (std::map<Memory, PoolBounds>::const_iterator it =
                 dynamic_pool_bounds.begin();
             it != dynamic_pool_bounds.end(); it++)
        {
          if (it->second.is_bounded())
            continue;
          MemoryManager* manager = runtime->find_memory_manager(it->first);
          ...
          if (it->first.address_space() != target_proc.address_space())
            REPORT_LEGION_ERROR(
                ERROR_DEFERRED_ALLOCATION_FAILURE,
                "%s memory " IDFMT
                " is not visible from the target processor"
        ...
        if (!unbounded_pools.empty())
        {
          ...
          for (std::vector<MemoryManager*>::const_iterator it =
                   unbounded_pools.begin();
               it != unbounded_pools.end(); it++)
          {
      ...
      for (std::map<Memory, MemoryPool*>::const_iterator it =
               acquired_pools.begin();
           it != acquired_pools.end(); it++)
        delete it->second;
    }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      std::vector<ApUserEvent> unmap_events(regions.size());
      {
        ...
        execution_context->initialize_region_tree_contexts(
            clone_requirements, version_infos, unmap_events);
      ...
      if (!task_profiling_requests.empty())
      {
        ...
        for (std::vector<ProfilingMeasurementID>::const_iterator it =
                 task_profiling_requests.begin();
             it != task_profiling_requests.end(); it++)
        {
          if ((*it) < Mapping::PMID_LEGION_FIRST)
      ...
      if (!to_release.empty())
      {
        for (std::vector<Reservation>::const_iterator it = to_release.begin();
             it != to_release.end(); it++)
    ...
    //--------------------------------------------------------------------------
    {
      if (!leaf_task)
      {
        ...
        if (configuration.auto_tracing_enabled)
        {
          log_auto_trace.info(
              ...
              get_unique_id());
          inner_ctx = new AutoTracing<InnerContext>(
              configuration, runtime, this, get_depth(), v->is_inner(), regions,
      ...
      else
      {
        ...
        LeafContext* leaf_ctx = new LeafContext(
            runtime, this, std::move(leaf_memory_pools), inline_task);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      oargs->task->runtime->order_concurrent_task_launch(
    ...
    //--------------------------------------------------------------------------
    {
      ...
      {
        legion_free(
            PREDICATE_ALLOC, predicate_false_result, predicate_false_size);
        predicate_false_result = NULL;
        predicate_false_size = 0;
      }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (output.verify_correctness)
      {
        ...
        runtime->forest->validate_slicing(
            internal_space, slice_spaces, this, mapper);
      }
      trigger_slices();
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!elide_future_return)
      {
        ...
        if (this->predicate_false_size > 0)
        {
          ...
          memcpy(
              this->predicate_false_result, rhs->predicate_false_result,
...
#pragma GCC diagnostic pop
        ...
        if (effects.exists())
        {
          if (reduction_instance_precondition.exists())
            effects = Runtime::merge_events(
                NULL, effects, reduction_instance_precondition);
        }
        else
    ...
    //--------------------------------------------------------------------------
    IndividualTask::IndividualTask(Runtime* rt) : SingleTask(rt)
    //--------------------------------------------------------------------------
    { }

    //--------------------------------------------------------------------------
    IndividualTask::IndividualTask(const IndividualTask& rhs) : SingleTask(NULL)
    //--------------------------------------------------------------------------
    {
    ...
    //--------------------------------------------------------------------------
    {
      FutureImpl* impl = new FutureImpl(
          parent_ctx, runtime, true /*register*/,
    ...
    //--------------------------------------------------------------------------
    {
      if (runtime->check_privileges && !is_top_level_task() && !local_function)
        ...
        if (predicate_false_future.impl == NULL)
        {
          if (predicate_false_size > 0)
            result.impl->set_local(
                predicate_false_result, predicate_false_size, false /*own*/);
          else
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else
      {
        ...
        if (elide_future_return)
        {
          if ((instance != NULL) && !instance->defer_deletion(effects))
            delete instance;
        }
        else
        {
    ...
    //--------------------------------------------------------------------------
    PointTask::PointTask(Runtime* rt) : SingleTask(rt)
    //--------------------------------------------------------------------------
    { }

    //--------------------------------------------------------------------------
    PointTask::PointTask(const PointTask& rhs) : SingleTask(NULL)
    //--------------------------------------------------------------------------
    {
      // should never be called
      assert(false);
    ...
    //--------------------------------------------------------------------------
    void PointTask::report_interfering_requirements(
        unsigned idx1, unsigned idx2)
    //--------------------------------------------------------------------------
    {
      switch (index_point.get_dim())
      {
        case 1:
          {
            REPORT_LEGION_ERROR(
                ERROR_ALIASED_REGION_REQUIREMENTS,
                "Aliased and interfering region requirements for "
                ...
                idx1, idx2, index_point[0], get_task_name(), get_unique_id(),
                parent_ctx->get_task_name(), parent_ctx->get_unique_id());
            break;
          }
#if LEGION_MAX_DIM >= 2
        case 2:
          {
            REPORT_LEGION_ERROR(
                ...
                idx1, idx2, index_point[0], index_point[1], get_task_name(),
        ...
        case 3:
          {
            REPORT_LEGION_ERROR(
                ...
                " (UID %lld) in parent task %s (UID %lld) are interfering.",
                idx1, idx2, index_point[0], index_point[1], index_point[2],
                get_task_name(), get_unique_id(), parent_ctx->get_task_name(),
                parent_ctx->get_unique_id());
            break;
          }
        ...
        case 4:
          {
            REPORT_LEGION_ERROR(
                ...
                "interfering.",
                idx1, idx2, index_point[0], index_point[1], index_point[2],
                index_point[3], get_task_name(), get_unique_id(),
                parent_ctx->get_task_name(), parent_ctx->get_unique_id());
            break;
          }
#endif
        ...
        case 5:
          {
            REPORT_LEGION_ERROR(
                ...
                "interfering.",
                idx1, idx2, index_point[0], index_point[1], index_point[2],
                index_point[3], index_point[4], get_task_name(),
                get_unique_id(), parent_ctx->get_task_name(),
                parent_ctx->get_unique_id());
            break;
          }
        ...
        case 6:
          {
            REPORT_LEGION_ERROR(
                ...
                "%lld) are interfering.",
                idx1, idx2, index_point[0], index_point[1], index_point[2],
                index_point[3], index_point[4], index_point[5], get_task_name(),
                get_unique_id(), parent_ctx->get_task_name(),
                parent_ctx->get_unique_id());
            break;
          }
        ...
        case 7:
          {
            REPORT_LEGION_ERROR(
                ...
                "%lld) are interfering.",
                idx1, idx2, index_point[0], index_point[1], index_point[2],
                index_point[3], index_point[4], index_point[5], index_point[6],
                get_task_name(), get_unique_id(), parent_ctx->get_task_name(),
                parent_ctx->get_unique_id());
            break;
          }
        ...
        case 8:
          {
            REPORT_LEGION_ERROR(
                ...
                "%s (UID %lld) are interfering.",
                idx1, idx2, index_point[0], index_point[1], index_point[2],
                index_point[3], index_point[4], index_point[5], index_point[6],
                index_point[7], get_task_name(), get_unique_id(),
                parent_ctx->get_task_name(), parent_ctx->get_unique_id());
            break;
          }
#endif
        ...
        case 9:
          {
            REPORT_LEGION_ERROR(
                ...
                idx1, idx2, index_point[0], index_point[1], index_point[2],
                index_point[3], index_point[4], index_point[5], index_point[6],
                index_point[7], index_point[8], get_task_name(),
                get_unique_id(), parent_ctx->get_task_name(),
                parent_ctx->get_unique_id());
            break;
          }
    ...
    //--------------------------------------------------------------------------
    ShardTask::ShardTask(
        Runtime* rt, SingleTask* source, InnerContext* parent,
    ...
    //--------------------------------------------------------------------------
    ShardTask::ShardTask(
        Runtime* rt, InnerContext* parent, Deserializer& derez,
        ShardManager* manager, ShardID id, Processor proc, VariantID variant)
      : SingleTask(rt), shard_id(id)
    //--------------------------------------------------------------------------
    {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else
      {
        ...
        execution_context = new LeafContext(
            runtime, this, std::move(leaf_memory_pools), inline_task);
        execution_context->add_base_gc_ref(SINGLE_TASK_REF);
        leaf_memory_pools.clear();
      }
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (configuration.auto_tracing_enabled)
      {
        ...
        repl_ctx = new AutoTracing<ReplicateContext>(
            configuration, runtime, this, get_depth(), false /*is inner*/,
            regions, output_regions, parent_req_indexes, virtual_mapped,
            task_priority, execution_fence_event, shard_manager,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      defer_perform_mapping(
          RtEvent::NO_RT_EVENT, NULL /*must epoch*/, 0 /*invocation count*/);
    }
    ...
    //--------------------------------------------------------------------------
    IndexTask::IndexTask(Runtime* rt) : MultiTask(rt)
    //--------------------------------------------------------------------------
    { }

    //--------------------------------------------------------------------------
    IndexTask::IndexTask(const IndexTask& rhs) : MultiTask(NULL)
    //--------------------------------------------------------------------------
    {
      // should never be called
      assert(false);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < output_regions.size(); ++idx)
      {
        ...
        IndexSpaceNode* parent =
            forest->get_node(output_regions[idx].parent.get_index_space());
#ifdef DEBUG_LEGION
      ...
      else
      {
        ...
        if (predicate_false_size > 0)
        {
          ...
          predicate_false_result =
              legion_malloc(PREDICATE_ALLOC, predicate_false_size);
          memcpy(
              predicate_false_result, pred_arg.get_ptr(), predicate_false_size);
        }
      }
    }
          ...
          if (internal_space.exists())
          {
            ...
            else
            {
              for (Domain::DomainPointIterator itr(local_domain); itr; itr++)
              {
                ...
                if (predicate_false_size > 0)
                  f.impl->set_local(
                      predicate_false_result, predicate_false_size,
                      false /*own*/);
                else
    ...
    bool IndexTask::perform_mapping(
        MustEpochOp* owner /*=NULL*/, const DeferMappingArgs* args /*=NULL*/)
    //--------------------------------------------------------------------------
    {
    ...
    //--------------------------------------------------------------------------
    {
      FutureMapImpl* result = new FutureMapImpl(
          ctx, this, this->launch_space, runtime,
    ...
    void IndexTask::return_slice_complete(
        unsigned points, ApEvent slice_effect, void* metadata /*= NULL*/,
        size_t metasize /*= 0*/)
    //--------------------------------------------------------------------------
    {
    ...
    //--------------------------------------------------------------------------
    {
           ...
           oit != point_requirements.end(); oit++)
      {
        ...
        for (std::set<std::pair<unsigned, unsigned>>::const_iterator it =
                 local_interfering.begin();
             it != local_interfering.end(); it++)
        {
              ...
              case 1:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      "in parent task %s (UID %lld) are interfering.",
                      it->first, point[0], it->second, oit->first[0],
                      get_task_name(), get_unique_id(),
                      parent_ctx->get_task_name(), parent_ctx->get_unique_id());
                  break;
                }
              ...
              case 2:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      "interfering.",
                      it->first, point[0], point[1], it->second, oit->first[0],
              ...
              case 3:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      "task %s (UID %lld) are interfering.",
                      it->first, point[0], point[1], point[2], it->second,
              ...
              case 4:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      " task %s (UID %lld) are interfering.",
                      it->first, point[0], point[1], point[2], point[3],
                      it->second, oit->first[0], oit->first[1], oit->first[2],
                      oit->first[3], get_task_name(), get_unique_id(),
                      parent_ctx->get_task_name(), parent_ctx->get_unique_id());
                  break;
                }
#endif
              ...
              case 5:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      "in parent task %s (UID %lld) are interfering.",
                      it->first, point[0], point[1], point[2], point[3],
                      point[4], it->second, oit->first[0], oit->first[1],
              ...
              case 6:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      "are interfering.",
                      it->first, point[0], point[1], point[2], point[3],
                      point[4], point[5], it->second, oit->first[0],
                      oit->first[1], oit->first[2], oit->first[3],
                      oit->first[4], oit->first[5], get_task_name(),
                      get_unique_id(), parent_ctx->get_task_name(),
                      parent_ctx->get_unique_id());
                  break;
                }
              ...
              case 7:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      "are interfering.",
                      it->first, point[0], point[1], point[2], point[3],
                      point[4], point[5], point[6], it->second, oit->first[0],
                      oit->first[1], oit->first[2], oit->first[3],
              ...
              case 8:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      "are interfering.",
                      it->first, point[0], point[1], point[2], point[3],
                      point[4], point[5], point[6], point[7], it->second,
                      oit->first[0], oit->first[1], oit->first[2],
                      oit->first[3], oit->first[4], oit->first[5],
                      oit->first[6], oit->first[7], get_task_name(),
                      get_unique_id(), parent_ctx->get_task_name(),
                      parent_ctx->get_unique_id());
                  break;
                }
              ...
              case 9:
                {
                  REPORT_LEGION_ERROR(
                      ...
                      it->second, oit->first[0], oit->first[1], oit->first[2],
                      oit->first[3], oit->first[4], oit->first[5],
                      oit->first[6], oit->first[7], oit->first[8],
                      get_task_name(), get_unique_id(),
                      parent_ctx->get_task_name(), parent_ctx->get_unique_id());
                  break;
                }
    ...
    //--------------------------------------------------------------------------
    SliceTask::SliceTask(Runtime* rt) : MultiTask(rt)
    //--------------------------------------------------------------------------
    { }

    //--------------------------------------------------------------------------
    SliceTask::SliceTask(const SliceTask& rhs) : MultiTask(NULL)
    //--------------------------------------------------------------------------
    {
      // should never be called
      assert(false);
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!elide_future_return)
      {
        if (redop == 0)
          future_map =
              FutureMapImpl::unpack_future_map(runtime, derez, parent_ctx);
        // Unpack the predicate false infos
        predicate_false_future = FutureImpl::unpack_future(runtime, derez);
        derez.deserialize(predicate_false_size);
        if (predicate_false_size > 0)
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < logical_regions.size(); idx++)
      {
        ...
        function->project_points(
            req, idx, runtime, index_domain, points,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!is_remote())
      {
        ...
        index_owner->check_point_requirements(
            point->index_point, point_regions);
#endif
    ...
    //--------------------------------------------------------------------------
    /*static*/ void SliceTask::handle_slice_return(
        Runtime* rt, Deserializer& derez)
    //--------------------------------------------------------------------------
    {
      DerezCheck z(derez);
