...
#undef LEGION_OPERATION_NAMES
    ...
    //--------------------------------------------------------------------------
    void Operation::analyze_region_requirements(IndexSpaceNode *launch_space,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      std::vector<Operation*> to_notify;
      {
        ...
        if (runtime->resilient_mode)
        {
          ...
          to_notify.insert(to_notify.end(), verification_notifications.begin(),
      ...
      for (unsigned idx = 0; idx < src_requirements.size(); idx++)
      {
        ...
        if (idx >= dst_indirect_requirements.size())
        {
          if (idx >= src_indirect_requirements.size())
          {
            ...
            if (!runtime->check_types(src_space.get_type_tag(),
          ...
          else
          {
            ...
            IndexSpace dst_space = 
              dst_requirements[idx].parent.get_index_space();
            bool diff_dims = false;
            if (!runtime->check_types(src_indirect_space.get_type_tag(),
                                           dst_space.get_type_tag(), diff_dims))
              Exception(DYNAMIC_TYPE_EXCEPTION, this)
                << "Copy index space mismatch at index " << idx
                << " of cross-region " << *this
          ...
          if (idx >= src_indirect_requirements.size())
          {
            ...
            if (!runtime->check_types(src_space.get_type_tag(), 
              ...
              Exception(DYNAMIC_TYPE_EXCEPTION, this)
                ...
                << (diff_dims ? "numbers of dimensions." : "coordinate types.");
          }
          else
          {
            // Indirect copy
    ...
    //--------------------------------------------------------------------------
    {
      ...
      for (unsigned idx = 0; idx < dst_requirements.size(); idx++)
      {
        ...
        perform_versioning_analysis(offset + idx, dst_requirements[idx],
      ...
      if (mapper->request_valid_instances)
      {
        ...
        if (!src_indirect_requirements.empty())
        {
          ...
          for (unsigned idx = 0; idx < src_indirect_requirements.size(); idx++)
          {
            InstanceSet valid_instances;
            FieldMaskSet<ReplicatedView> collectives;
            physical_premap_region(offset+idx, 
      ...
      if (scatter_targets == NULL)
      {
        if (gather_targets == NULL)
        {
          ...
          copy_post = copy_across( 
              ...
              src_targets, dst_targets, across_sources, index,
              trace_info.dst_index, init_precondition, src_ready, dst_ready,
              predication_guard, atomic_locks[index],
        ...
        else
        {
          ...
          copy_post = gather_across(
              ...
              src_targets, (*gather_targets), dst_targets, index, 
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID,
                             ...
                             "space %lld for index %d of %s requirements "
                             "of copy operation (ID %lld)",
                             bad_field, sp.get_id(), idx, req_kind, unique_op_id)
            break;
          }
...
#ifdef DEBUG_ELGION
      assert(idx < scatter_versions.size());
#endif
    ...
    //--------------------------------------------------------------------------
    {
      ...
      sharding_space = launcher.sharding_space;
      collective_exchanges.resize(std::max(src_indirect_requirements.size(), 
            dst_indirect_requirements.size()));
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (source)
      {
        const std::vector<IndirectRecord> &records = src_indirect_records[index];
      ...
      {
        const std::vector<IndirectRecord> &records = dst_indirect_records[index];
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (!src_indirect_requirements.empty())
      {
        ...
        gather_versions.resize(src_indirect_requirements.size());
        for (unsigned idx = 0; idx < src_indirect_requirements.size(); idx++)
          perform_versioning_analysis(offset + idx, 
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID,
                             "Field %d is not a valid field of field "
                             "space %llu of requirement for acquire "
                             "operation (ID %lld)",
    ...
    //--------------------------------------------------------------------------
    {
      ...
      ApEvent release_complete = 
        release_restrictions(requirement, version_info,
                                              0/*idx*/, init_precondition,
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID,
                             "Field %d is not a valid field of field "
                             "space %llu of requirement for release "
                             "operation (ID %lld)",
                             bad_field, sp.get_id(), unique_op_id)
            break;
          }
...
#undef DIMFUNC
        ...
        if (!internal_dependences.empty())
        {
          ...
          if (finder != internal_dependences.end())
          {
                  ...
                  finder->second.begin(); it != finder->second.end(); it++)
            {
              ...
              if (runtime->are_disjoint_tree_only(src_node, 
                                                  dst_node, dummy))
                continue;
...
#endif
        partition_node = runtime->get_node(
    ...
    ApEvent DependentPartitionOp::ByFieldThunk::perform(
           DependentPartitionOp *op, FieldID fid,
    ...
    //--------------------------------------------------------------------------
    {
      return op->create_partition_by_preimage(fid, pid, projection, 
    ...
    //--------------------------------------------------------------------------
    {
      return op->create_partition_by_preimage_range(fid, pid,projection,
                           instances, remote_targets, results, instances_ready);
    }
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID_FIELD,
                            "Field %d is not a valid field of field "
                             "space %llu for dependent partitioning op "
                             "(ID %lld)", bad_field, sp.get_id(), unique_op_id)
            break;
          }
        case ERROR_INVALID_INSTANCE_FIELD:
          {
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID,
                             "Field %d is not a valid field of field "
                             "space %llu for fill operation (ID %lld)",
                             bad_field, sp.get_id(), unique_op_id)
            break;
          }
        case ERROR_INVALID_INSTANCE_FIELD:
    ...
    //--------------------------------------------------------------------------
    {
      for (unsigned idx1 = 0; idx1 < points.size(); idx1++)
      {
        ...
        for (unsigned idx2 = 0; idx2 < idx1; idx2++)
        {
          ...
          if (!runtime->are_disjoint(req1.region.get_index_space(), 
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID,
                             "Field %d is not a valid field of field "
                             "space %llu for discard operation (ID %lld)",
                             bad_field, sp.get_id(), unique_op_id)
            break;
          }
        case ERROR_INVALID_INSTANCE_FIELD:
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID,
                             "Field %d is not a valid field of field "
                             "space %llu for attach operation (ID %lld)",
                             bad_field, sp.get_id(), unique_op_id)
            break;
          }
        case ERROR_INVALID_INSTANCE_FIELD:
        ...
        case ERROR_FIELD_SPACE_FIELD_MISMATCH:
          {
            ...
            REPORT_LEGION_ERROR(ERROR_FIELD_NOT_VALID,
                             "Field %d is not a valid field of field "
                             "space %llu for index attach operation (ID %lld)",
                             bad_field, sp.get_id(), unique_op_id)
            break;
          }
        case ERROR_INVALID_INSTANCE_FIELD:
