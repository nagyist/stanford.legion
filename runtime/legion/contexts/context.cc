...
namespace Legion {
  namespace Internal {
    ...
    //--------------------------------------------------------------------------
    TaskContext::TaskContext(SingleTask *owner, int d,
    ...
    //--------------------------------------------------------------------------
    bool TaskContext::safe_cast(IndexSpace handle,
    ...
    //--------------------------------------------------------------------------
    {
      ...
      else
      {
        ...
        if (our_tid != req.partition.get_tree_id())
          return false;
        if (!runtime->has_partition_path(our_space,
...
#ifdef DEBUG_LEGION
        runtime->decrement_total_outstanding_tasks(owner_task_id,false/*meta*/);
#else
    ...
    //--------------------------------------------------------------------------
    FutureMap TaskContext::predicate_index_task_false(IndexSpace launch_space,
