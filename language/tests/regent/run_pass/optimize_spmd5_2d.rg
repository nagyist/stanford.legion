...
-- using ForList loops over a 2d index space.
...
task main()
  ...
  var is_part = ispace(int2d, { x = 2, y = 2 })

  var p = partition(equal, r, is_part)

  var cq = c.legion_domain_point_coloring_create()
  c.legion_domain_point_coloring_color_domain(
    cq, (int2d { x=0, y=0 }):to_domain_point(),
    c.legion_domain_from_rect_2d(rect2d { lo={ x=0, y=0 }, hi={ x=2, y=3 }}))
  c.legion_domain_point_coloring_color_domain(
    cq, (int2d { x=0, y=1 }):to_domain_point(),
    c.legion_domain_from_rect_2d(rect2d { lo={ x=1, y=0 }, hi={ x=3, y=2 }}))
  c.legion_domain_point_coloring_color_domain(
    cq, (int2d { x=1, y=0 }):to_domain_point(),
    c.legion_domain_from_rect_2d(rect2d { lo={ x=0, y=3 }, hi={ x=3, y=3 }}))
  c.legion_domain_point_coloring_color_domain(
    cq, (int2d { x=1, y=1 }):to_domain_point(),
    c.legion_domain_from_rect_2d(rect2d { lo={ x=3, y=0 }, hi={ x=3, y=3 }}))
  ...
  regentlib.assert(r[{ x=0, y=0 }].a ==  9471, "test failed")
  regentlib.assert(r[{ x=0, y=3 }].a ==  9450, "test failed")
  regentlib.assert(r[{ x=1, y=1 }].a == 10367, "test failed")
  regentlib.assert(r[{ x=2, y=0 }].a ==  9742, "test failed")
  regentlib.assert(r[{ x=3, y=2 }].a ==  9653, "test failed")
  regentlib.assert(r[{ x=3, y=3 }].a == 10723, "test failed")
end
