...
static int ftruncate(int fd, off_t size) { return _chsize_s(fd, (__int64)size); }
...
namespace Realm {
  ...
  FileMemory::FileMemory(RuntimeImpl *_runtime_impl, Memory _me)
    : MemoryImpl(_runtime_impl, _me, 0 /*no memory space*/, MKIND_FILE, Memory::FILE_MEM,
                 0)
  {}

  FileMemory::~FileMemory(void) {}

  void FileMemory::get_bytes(off_t offset, void *dst, size_t size) { assert(0); }
  ...
  void FileMemory::put_bytes(off_t offset, const void *src, size_t) { assert(0); }

  void FileMemory::put_bytes(ID::IDType inst_id, off_t offset, const void *src,
                             size_t size)
  {
    assert(0);
  }
  ...
  void FileMemory::unregister_external_resource(RegionInstanceImpl *inst)
  {
    ...
    if(ret == -1) {
      log_disk.warning()
          << "file failed to close cleanly, disk contents may be corrupted";
    }
  }

  MemoryImpl::AllocationResult
  FileMemory::allocate_storage_immediate(RegionInstanceImpl *inst, bool need_alloc_result,
                                         bool poisoned, TimeLimit work_until)
  {
    // we can't actually allocate anything in a FileMemory
    if(inst->metadata.ext_resource)
      log_inst.warning() << "attempt to register non-file resource: mem=" << me
                         << " resource=" << *(inst->metadata.ext_resource);
    else
      log_inst.warning() << "attempt to allocate memory in file memory: layout="
  ...
  void FileMemory::release_storage_immediate(RegionInstanceImpl *inst, bool poisoned,
