...
namespace Realm {
  ...
  namespace UCP {
    ...
    {
      void *ptr_to_align = (char *)(chunk + 1) + sizeof(Elem) + alignment_offset;

      uintptr_t padding = PADDING_TO_ALIGN((uintptr_t)ptr_to_align, alignment);
    ...
    bool MPool::validate_config()
    {
      ...
      if(obj_size > max_chunk_size) {
        log_ucp_mp.error() << "mpool max chunk size " << max_chunk_size
    ...
    MPool::MPool(std::string name_, bool leak_check_, size_t obj_size_, size_t alignment_,
                 size_t alignment_offset_, size_t objs_per_chunk_, size_t init_num_objs_,
                 size_t max_objs_, size_t max_chunk_size_, double expand_factor_,
                 chunk_alloc_t chunk_alloc_, void *chunk_alloc_arg_,
                 chunk_release_t chunk_release_, void *chunk_release_arg_,
                 obj_init_t obj_init_, void *obj_init_arg_, obj_cleanup_t obj_cleanup_,
                 void *obj_cleanup_arg_)
      : name(name_)
      ...
      , obj_cleanup_arg(obj_cleanup_arg_)
    {
      ...
      if(!expand(init_num_objs)) {
        log_ucp_mp.error() << "mpool initial expansion to " << init_num_objs
                           << " objects failed";
        abort();
      }

      log_ucp_mp.info() << "mpool " << this << " " << name << " initialized"
                        << " obj_size " << obj_size << ", alignment " << alignment
                        << ", alignment_offset " << alignment_offset
                        << ", objs_per_chunk " << objs_per_chunk << ", init_num_objs "
                        << init_num_objs << ", max_objs " << max_objs
                        << ", max_chunk_size " << max_chunk_size << ", expand_factor "
                        << expand_factor;
    }
    ...
    MPool::~MPool()
    {
      ...
      log_ucp_mp.info() << "destructing mpool " << name << " num_objs " << num_objs;
      ...
      if(leak_check) {
        ...
        for(chunk = chunks; chunk != nullptr; chunk = chunk->next) {
          for(size_t i = 0; i < chunk->num_objs; i++) {
            ...
            if(elem->mp != nullptr) {
              log_ucp_mp.warning()
                  << "mpool object not returned"
                  << " mp " << this << " " << name << " obj " << (void *)(elem + 1);
            }
          }
        }
      }
    ...
    bool MPool::expand(size_t ext_objs)
    {
      ...
      if(num_objs == max_objs) {
        log_ucp_mp.error() << "mpool expand failed."
                           << " Maximum number of objects reached " << max_objs;
        return false;
      }
      if(ext_objs > max_objs || num_objs > max_objs - ext_objs) {
    ...
    // class VMPool
    //
    VMPool::VMPool(std::string name_, bool leak_check_, size_t max_obj_size_,
                   size_t alignment_, size_t objs_per_chunk_, size_t init_num_objs_,
                   size_t max_objs_, size_t max_chunk_size_, double expand_factor_,
                   chunk_alloc_t chunk_alloc_, void *chunk_alloc_arg_,
                   chunk_release_t chunk_release_, void *chunk_release_arg_,
                   obj_init_t obj_init_, void *obj_init_arg_, obj_cleanup_t obj_cleanup_,
      ...
      , alignment(alignment_)
    {
      ...
      mp = new MPool(name_, leak_check_, mpool_max_obj_size, alignof(Elem), 0,
                     objs_per_chunk_, init_num_objs_, max_objs_, max_chunk_size_,
                     expand_factor_, chunk_alloc_, chunk_alloc_arg_, chunk_release_,
                     chunk_release_arg_, obj_init_, obj_init_arg_, obj_cleanup_,
                     obj_cleanup_arg_);
    }

    VMPool::~VMPool() { delete mp; }
    ...
    void *VMPool::get(size_t size)
    {
      ...
      uintptr_t addr_aligned = ALIGN_UP_POW2(addr_cached + metadata_size, alignment);
      ...
      if(addr_aligned + size <= limit_cached) {
        ...
        addr_aligned = ALIGN_UP_POW2(parent_obj + metadata_size, alignment);
      ...
      elem = (Elem *)(void *)ALIGN_DOWN_POW2((uintptr_t)(void *)elem, alignof(Elem));
      elem->vmp = this;
    ...
    bool VMPool::expand(size_t ext_objs) { return mp->expand(ext_objs); }
