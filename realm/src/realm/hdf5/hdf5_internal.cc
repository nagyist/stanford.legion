...
namespace Realm {
  ...
  namespace HDF5 {
    ...
    void HDF5Memory::get_bytes(off_t offset, void *dst, size_t size) { assert(0); }
    ...
    void HDF5Memory::put_bytes(off_t offset, const void *src, size_t size) { assert(0); }
    ...
    {
      {
        ExternalHDF5Resource *res =
            dynamic_cast<ExternalHDF5Resource *>(inst->metadata.ext_resource);
        if(res) {
    ...
    HDF5Memory::allocate_storage_immediate(RegionInstanceImpl *inst,
                                           ...
                                           TimeLimit work_until)
    {
      ...
      if(inst->metadata.ext_resource)
        log_inst.warning() << "attempt to register non-hdf5 resource: mem=" << me
                           << " resource=" << *(inst->metadata.ext_resource);
      else
        log_inst.warning() << "attempt to allocate memory in hdf5 memory: layout="
                           << *(inst->metadata.layout);
    ...
    void HDF5Memory::release_storage_immediate(RegionInstanceImpl *inst, bool poisoned,
                                               TimeLimit work_until)
    {
    ...
    int AddressInfoHDF5::set_rect(const RegionInstanceImpl *inst,
                                  ...
                                  size_t field_offset, int ndims,
                                  const int64_t lo[/*ndims*/],
                                  const int64_t hi[/*ndims*/], const int order[/*ndims*/])
    {
      // get hdf5 metadata for instance and piece
      const ExternalHDF5Resource *res =
          checked_cast<ExternalHDF5Resource *>(inst->metadata.ext_resource);
    ...
    HDF5XferDes::HDF5XferDes(uintptr_t _dma_op, Channel *_channel, NodeID _launch_node,
                             XferDesID _guid,
    ...
    bool HDF5XferDes::request_available() { return !req_in_use; }
    ...
    long HDF5XferDes::get_requests(Request **requests, long nr)
    {
      ...
      while((idx < nr) && request_available()) {
        ...
        if(in_port->peer_guid != XFERDES_NO_GUID) {
          size_t pre_max =
              in_port->remote_bytes_total.load() - in_port->local_bytes_total;
          if(pre_max == 0) {
        ...
        TransferIterator *hdf5_iter =
            ((kind == XFER_HDF5_READ) ? in_port->iter : out_port->iter);
        TransferIterator *mem_iter =
            ((kind == XFER_HDF5_READ) ? out_port->iter : in_port->iter);
        ...
        size_t hdf5_bytes =
            hdf5_iter->step_custom(max_bytes, hdf5_info, true /*tentative*/);
        if(hdf5_bytes == 0) {
          // not enough space for even a single element - try again later
          break;
        }
        // TODO: support 2D/3D for memory side of an HDF transfer?
        size_t mem_bytes = mem_iter->step(hdf5_bytes, mem_info, 0, true /*tentative*/);
        ...
        new_req->mem_base =
            ((kind == XFER_HDF5_READ) ? out_port->mem : in_port->mem)
                ->get_direct_ptr(mem_info.base_offset, mem_info.bytes_per_chunk);
        ...
        HDF5Dataset *dset;
        {
          ...
          if(it != datasets.end()) {
            ...
            dset =
                HDF5Dataset::open(hdf5_info.filename->c_str(),
                                  hdf5_info.dsetname->c_str(), (kind == XFER_HDF5_READ));
            assert(dset != 0);
        ...
        CHECK_HDF5(new_req->mem_space_id =
                       H5Screate_simple(mem_dims.size(), mem_dims.data(), NULL));
        // std::vector<hsize_t> mem_start(DIM, 0);
        // CHECK_HDF5( H5Sselect_hyperslab(new_req->mem_space_id, H5S_SELECT_SET,
        // ms_start, NULL, count, NULL) );
        CHECK_HDF5(new_req->file_space_id = H5Scopy(dset->dspace_id));
        CHECK_HDF5(H5Sselect_hyperslab(new_req->file_space_id, H5S_SELECT_SET,
    ...
    bool HDF5XferDes::progress_xd(HDF5Channel *channel, TimeLimit work_until)
    {
      ...
      while(true) {
        ...
        if(output_control.current_io_port >= 0) {
          ...
          // always ask the HDF5 size for a step first
          size_t hdf5_bytes =
              out_port->iter->step_custom(max_bytes, hdf5_info, false /*!tentative*/);
          assert(hdf5_bytes > 0);
          ...
          CHECK_HDF5(mem_space_id =
                         H5Screate_simple(mem_dims.size(), mem_dims.data(), 0));
          ...
          CHECK_HDF5(H5Dwrite(dset->dset_id, dset->dtype_id, mem_space_id, file_space_id,
                              H5P_DEFAULT, fill_data));

          CHECK_HDF5(H5Sclose(mem_space_id));
          CHECK_HDF5(H5Sclose(file_space_id));

          update_bytes_write(output_control.current_io_port, out_port->local_bytes_total,
                             hdf5_bytes);
        ...
        if(output_control.control_port_idx >= 0)
          done = ((output_control.remaining_count == 0) && output_control.eos_received);
    ...
    void HDF5XferDes::flush()
    {
      ...
      for(DatasetMap::const_iterator it = datasets.begin(); it != datasets.end(); ++it)
        it->second->close();
      datasets.clear();
    }
    ...
    HDF5Channel::HDF5Channel(BackgroundWorkManager *bgwork)
      : SingleXDQChannel<HDF5Channel, HDF5XferDes>(bgwork, XFER_NONE /*FIXME*/,
                                                   "hdf5 channel")
    {
      ...
      add_path(Memory::HDF_MEM, false, local_cpu_mems, bw, latency, frag_overhead,
               XFER_HDF5_READ);

      add_path(local_cpu_mems, Memory::HDF_MEM, false, bw, latency, frag_overhead,
               XFER_HDF5_WRITE);

      // also indicate willingness to handle fills to HDF5 (src == NO_MEMORY)
      add_path(Memory::NO_MEMORY, Memory::HDF_MEM, false, bw, latency, frag_overhead,
               XFER_HDF5_WRITE);
    }
    ...
    XferDes *
    HDF5Channel::create_xfer_des(uintptr_t dma_op, NodeID launch_node, XferDesID guid,
                                 const std::vector<XferDesPortInfo> &inputs_info,
                                 const std::vector<XferDesPortInfo> &outputs_info,
                                 ...
                                 size_t fill_total)
    {
      ...
      return new HDF5XferDes(dma_op, this, launch_node, guid, inputs_info, outputs_info,
                             priority, fill_data, fill_size);
    }
    ...
    long HDF5Channel::submit(Request **requests, long nr)
    {
      ...
      for(long i = 0; i < nr; i++) {
        ...
        if(req->xd->kind == XFER_HDF5_READ)
          CHECK_HDF5(H5Dread(req->dataset_id, req->datatype_id, req->mem_space_id,
                             req->file_space_id, H5P_DEFAULT, req->mem_base));
        else
          CHECK_HDF5(H5Dwrite(req->dataset_id, req->datatype_id, req->mem_space_id,
                              req->file_space_id, H5P_DEFAULT, req->mem_base));
