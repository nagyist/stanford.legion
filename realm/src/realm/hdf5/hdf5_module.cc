...
namespace Realm {
  ...
  namespace HDF5 {
    ...
    /*static*/ HDF5Dataset *HDF5Dataset::open(const char *filename, const char *dsetname,
                                              bool read_only)
    {
      ...
      if(it == file_cache.end()) {
        ...
        CHECK_HDF5(f.file_id =
                       H5Fopen(filename, (open_as_rw ? H5F_ACC_RDWR : H5F_ACC_RDONLY),
                               H5P_DEFAULT));
        log_hdf5.info() << "H5Fopen(\"" << filename << "\", "
                        << (open_as_rw ? "H5F_ACC_RDWR" : "H5F_ACC_RDONLY")
                        << ") = " << f.file_id;
        if(f.file_id < 0)
          return 0;
        f.usage_count = 0;
      ...
      while(true) {
        ...
        log_hdf5.info() << "H5Gopen2(" << loc_id << ", \"" << grpname
                        << "\") = " << grp_id;
      ...
      log_hdf5.info() << "H5Dopen2(" << it->second.file_id << ", \"" << dsetname
                      << "\") = " << dset_id;
      if(loc_id != it->second.file_id)
        CHECK_HDF5(H5Gclose(loc_id));
      if(dset_id < 0)
    ...
    void HDF5Dataset::flush() { CHECK_HDF5(H5Fflush(file_id, H5F_SCOPE_GLOBAL)); }

    void HDF5Dataset::close()
    {
      // find our file in the cache
      HDF5FileCache::iterator it = file_cache.begin();
      while((it != file_cache.end()) && (it->second.file_id != file_id))
        ++it;
      ...
      it->second.usage_count--;
      if((it->second.usage_count == 0) && (file_cache.size() > Config::max_open_files)) {
    ...
    /*static*/ Module *HDF5Module::create_module(RuntimeImpl *runtime)
    {
      ...
      {
        herr_t err;
        err = H5get_libversion(&m->version_major, &m->version_minor, &m->version_rel);
        assert(err == 0);
    ...
    //  complete
    void HDF5Module::initialize(RuntimeImpl *runtime) { Module::initialize(runtime); }
    ...
    void HDF5Module::cleanup(void)
    {
      ...
      for(HDF5FileCache::iterator it = file_cache.begin(); it != file_cache.end(); ++it) {
        if(it->second.usage_count > 0)
          log_hdf5.warning() << "nonzero usage count on file \"" << it->first.first
                             << "\": " << it->second.usage_count;
        log_hdf5.info() << "H5Fclose(" << it->second.file_id << ")";
        CHECK_HDF5(H5Fclose(it->second.file_id));
      }
