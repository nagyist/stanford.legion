...
REALM_PUBLIC_API
int omp_get_num_procs(void) { return omp_get_max_threads(); }
...
void omp_set_num_threads(int num_threads)
{
  ...
  if(wi) {
    ...
    log_omp.warning() << "omp_set_num_threads(" << num_threads
                      << ") called on non-OpenMP Realm proessor - ignoring";
  }
}
};
...
void GOMP_parallel_start(void (*fnptr)(void *data), void *data, int nthreads)
{
  ...
  int idx = 1;
  for(std::set<int>::const_iterator it = worker_ids.begin(); it != worker_ids.end();
      ++it) {
...
REALM_PUBLIC_API
void GOMP_parallel(void (*fnptr)(void *data), void *data, unsigned nthreads,
                   unsigned int flags)
{
...
REALM_PUBLIC_API
bool GOMP_loop_static_start(long start, long end, long incr, long chunk, long *istart,
                            long *iend)
{
  Realm::ThreadPool::WorkerInfo *wi = Realm::ThreadPool::get_worker_info(true);
  if(!wi) {
  ...
  log_omp.debug() << "loop static start: start=" << start << " end=" << end
                  << " incr=" << incr << " chunk=" << chunk;

  int64_t span_start, span_end;
  bool more = wi->work_item->schedule.start_static(start, end, incr, chunk, wi->thread_id,
                                                   span_start, span_end);
  if(more) {
    *istart = span_start;
    *iend = span_end;
...
REALM_PUBLIC_API
bool GOMP_loop_dynamic_start(long start, long end, long incr, long chunk, long *istart,
                             long *iend)
{
  Realm::ThreadPool::WorkerInfo *wi = Realm::ThreadPool::get_worker_info(true);
  if(!wi) {
  ...
  log_omp.debug() << "loop dynamic start: start=" << start << " end=" << end
                  << " incr=" << incr << " chunk=" << chunk;
...
REALM_PUBLIC_API
bool GOMP_loop_ull_dynamic_start(bool up, uint64_t start, uint64_t end, uint64_t incr,
                                 uint64_t chunk, uint64_t *istart, uint64_t *iend)
{
  Realm::ThreadPool::WorkerInfo *wi = Realm::ThreadPool::get_worker_info(true);
  if(!wi) {
    // give back the whole loop and hope for the best
...
bool GOMP_loop_static_next(long *istart, long *iend)
{
  ...
  if(more) {
    *istart = span_start;
    *iend = span_end;
    log_omp.debug() << "loop static next: start=" << *istart << " end=" << *iend;
  } else
...
bool GOMP_loop_dynamic_next(long *istart, long *iend)
{
  ...
  log_omp.debug() << "loop dynamic next: pstart=" << *istart << " pend=" << *iend;
  ...
  if(more) {
    *istart = span_start;
    *iend = span_end;
    log_omp.debug() << "loop dynamic next: start=" << *istart << " end=" << *iend;
  } else
...
REALM_PUBLIC_API
void GOMP_critical_start(void) { GOMP_critical_name_start(0); }

REALM_PUBLIC_API
void GOMP_critical_end(void) { GOMP_critical_name_end(0); }
...
REALM_PUBLIC_API
void GOMP_atomic_start(void) { gomp_atomic_mutex.lock(); }

REALM_PUBLIC_API
void GOMP_atomic_end(void) { gomp_atomic_mutex.unlock(); }
};
#endif
...
namespace Realm {
  ...
  struct kmp_thunk {
    ...
    static void invoke_1(void *data)
    {
      ...
      kmp_int32 global_tid = 44;                  // what does this do?
      kmp_int32 local_tid = omp_get_thread_num(); // CHECK
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0]);
    }
    ...
    static void invoke_2(void *data)
    {
      ...
      kmp_int32 global_tid = 44;                  // what does this do?
      kmp_int32 local_tid = omp_get_thread_num(); // CHECK
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1]);
    }
    ...
    static void invoke_5(void *data)
    {
      ...
      kmp_int32 local_tid = omp_get_thread_num(); // CHECK
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         thunk->argv[2], thunk->argv[3], thunk->argv[4]);
    }
    ...
    static void invoke_6(void *data)
    {
      ...
      kmp_int32 local_tid = omp_get_thread_num(); // CHECK
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         thunk->argv[2], thunk->argv[3], thunk->argv[4], thunk->argv[5]);
    }
    ...
    static void invoke_9(void *data)
    {
      ...
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         thunk->argv[2], thunk->argv[3], thunk->argv[4], thunk->argv[5],
                         thunk->argv[6], thunk->argv[7], thunk->argv[8]);
    }
    ...
    static void invoke_10(void *data)
    {
      ...
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         thunk->argv[2], thunk->argv[3], thunk->argv[4], thunk->argv[5],
                         thunk->argv[6], thunk->argv[7], thunk->argv[8], thunk->argv[9]);
    }
    ...
    static void invoke_13(void *data)
    {
      ...
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         thunk->argv[2], thunk->argv[3], thunk->argv[4], thunk->argv[5],
                         thunk->argv[6], thunk->argv[7], thunk->argv[8], thunk->argv[9],
                         thunk->argv[10], thunk->argv[11], thunk->argv[12]);
    }
    ...
    static void invoke_16(void *data)
    {
      ...
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         ...
                         thunk->argv[6], thunk->argv[7], thunk->argv[8], thunk->argv[9],
                         thunk->argv[10], thunk->argv[11], thunk->argv[12],
                         thunk->argv[13], thunk->argv[14], thunk->argv[15]);
    }
    ...
    static void invoke_19(void *data)
    {
      ...
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         ...
                         thunk->argv[10], thunk->argv[11], thunk->argv[12],
                         thunk->argv[13], thunk->argv[14], thunk->argv[15],
                         thunk->argv[16], thunk->argv[17], thunk->argv[18]);
    }
    ...
    static void invoke_20(void *data)
    {
      ...
      kmp_int32 local_tid = omp_get_thread_num(); // CHECK
      (thunk->microtask)(
          &global_tid, &local_tid, thunk->argv[0], thunk->argv[1], thunk->argv[2],
          thunk->argv[3], thunk->argv[4], thunk->argv[5], thunk->argv[6], thunk->argv[7],
          thunk->argv[8], thunk->argv[9], thunk->argv[10], thunk->argv[11],
          thunk->argv[12], thunk->argv[13], thunk->argv[14], thunk->argv[15],
          thunk->argv[16], thunk->argv[17], thunk->argv[18], thunk->argv[19]);
    }
    ...
    static void invoke_22(void *data)
    {
      ...
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         ...
                         thunk->argv[13], thunk->argv[14], thunk->argv[15],
                         thunk->argv[16], thunk->argv[17], thunk->argv[18],
                         thunk->argv[19], thunk->argv[20], thunk->argv[21]);
    }
    ...
    static void invoke_23(void *data)
    {
      ...
      (thunk->microtask)(
          ...
          thunk->argv[12], thunk->argv[13], thunk->argv[14], thunk->argv[15],
          thunk->argv[16], thunk->argv[17], thunk->argv[18], thunk->argv[19],
          thunk->argv[20], thunk->argv[21], thunk->argv[22]);
    }
    ...
    static void invoke_24(void *data)
    {
      ...
      (thunk->microtask)(
          ...
          thunk->argv[12], thunk->argv[13], thunk->argv[14], thunk->argv[15],
          thunk->argv[16], thunk->argv[17], thunk->argv[18], thunk->argv[19],
          thunk->argv[20], thunk->argv[21], thunk->argv[22], thunk->argv[23]);
    }
    ...
    static void invoke_25(void *data)
    {
      ...
      (thunk->microtask)(&global_tid, &local_tid, thunk->argv[0], thunk->argv[1],
                         ...
                         thunk->argv[16], thunk->argv[17], thunk->argv[18],
                         thunk->argv[19], thunk->argv[20], thunk->argv[21],
                         thunk->argv[22], thunk->argv[23], thunk->argv[24]);
    }
    ...
    static void invoke_27(void *data)
    {
      ...
      (thunk->microtask)(
          ...
          thunk->argv[16], thunk->argv[17], thunk->argv[18], thunk->argv[19],
          thunk->argv[20], thunk->argv[21], thunk->argv[22], thunk->argv[23],
          thunk->argv[24], thunk->argv[25], thunk->argv[26]);
    }
    ...
    static void invoke_28(void *data)
    {
      ...
      (thunk->microtask)(
          ...
          thunk->argv[16], thunk->argv[17], thunk->argv[18], thunk->argv[19],
          thunk->argv[20], thunk->argv[21], thunk->argv[22], thunk->argv[23],
          thunk->argv[24], thunk->argv[25], thunk->argv[26], thunk->argv[27]);
    }
    ...
    static void invoke_31(void *data)
    {
      ...
      (thunk->microtask)(
          ...
          thunk->argv[20], thunk->argv[21], thunk->argv[22], thunk->argv[23],
          thunk->argv[24], thunk->argv[25], thunk->argv[26], thunk->argv[27],
          thunk->argv[28], thunk->argv[29], thunk->argv[30]);
    }
    ...
    static void invoke_32(void *data)
    {
      ...
      (thunk->microtask)(
          ...
          thunk->argv[20], thunk->argv[21], thunk->argv[22], thunk->argv[23],
          thunk->argv[24], thunk->argv[25], thunk->argv[26], thunk->argv[27],
          thunk->argv[28], thunk->argv[29], thunk->argv[30], thunk->argv[31]);
    }
  };
...
void __kmpc_push_num_threads(ident_t *loc, kmp_int32 global_tid, kmp_int32 num_threads)
{
  ...
  if(wi) {
    ...
    log_omp.warning() << "__kmpc_push_num_threads(" << num_threads
                      << ") called on non-OpenMP Realm proessor - ignoring";
  }
}
...
void __kmpc_fork_call(ident_t *loc, kmp_int32 argc, kmpc_micro microtask, ...)
{
  ...
  switch(argc) {
  case 0:
    invoker = &kmp_thunk::invoke_0;
    break;
  case 1:
    invoker = &kmp_thunk::invoke_1;
    break;
  case 2:
    invoker = &kmp_thunk::invoke_2;
    break;
  case 3:
    invoker = &kmp_thunk::invoke_3;
    break;
  case 4:
    invoker = &kmp_thunk::invoke_4;
    break;
  case 5:
    invoker = &kmp_thunk::invoke_5;
    break;
  case 6:
    invoker = &kmp_thunk::invoke_6;
    break;
  case 7:
    invoker = &kmp_thunk::invoke_7;
    break;
  case 8:
    invoker = &kmp_thunk::invoke_8;
    break;
  case 9:
    invoker = &kmp_thunk::invoke_9;
    break;
  case 10:
    invoker = &kmp_thunk::invoke_10;
    break;
  case 11:
    invoker = &kmp_thunk::invoke_11;
    break;
  case 12:
    invoker = &kmp_thunk::invoke_12;
    break;
  case 13:
    invoker = &kmp_thunk::invoke_13;
    break;
  case 14:
    invoker = &kmp_thunk::invoke_14;
    break;
  case 15:
    invoker = &kmp_thunk::invoke_15;
    break;
  case 16:
    invoker = &kmp_thunk::invoke_16;
    break;
  case 17:
    invoker = &kmp_thunk::invoke_17;
    break;
  case 18:
    invoker = &kmp_thunk::invoke_18;
    break;
  case 19:
    invoker = &kmp_thunk::invoke_19;
    break;
  case 20:
    invoker = &kmp_thunk::invoke_20;
    break;
  case 21:
    invoker = &kmp_thunk::invoke_21;
    break;
  case 22:
    invoker = &kmp_thunk::invoke_22;
    break;
  case 23:
    invoker = &kmp_thunk::invoke_23;
    break;
  case 24:
    invoker = &kmp_thunk::invoke_24;
    break;
  case 25:
    invoker = &kmp_thunk::invoke_25;
    break;
  case 26:
    invoker = &kmp_thunk::invoke_26;
    break;
  case 27:
    invoker = &kmp_thunk::invoke_27;
    break;
  case 28:
    invoker = &kmp_thunk::invoke_28;
    break;
  case 29:
    invoker = &kmp_thunk::invoke_29;
    break;
  case 30:
    invoker = &kmp_thunk::invoke_30;
    break;
  case 31:
    invoker = &kmp_thunk::invoke_31;
    break;
  case 32:
    invoker = &kmp_thunk::invoke_32;
    break;
  default:
  {
...
namespace Realm {
                                          ...
                                          typename make_signed<T>::type chunk)
  {
    ...
    if(!wi || (wi->num_threads == 1) || ((incr > 0) && (*plower > *pupper)) ||
    ...
    case 33 /* kmp_sch_static_chunked - (chunk ignored - TODO) */:
    {
      ...
      *pupper =
          *plower + incr * (whole + ((((T)(wi->thread_id)) < leftover) ? 1 : 0)) - 1;
  ...
  template <typename T>
  void kmpc_dispatch_init(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype, T lb,
                          T ub, typename make_signed<T>::type st,
                          typename make_signed<T>::type chunk)
  {
    Realm::ThreadPool::WorkerInfo *wi = Realm::ThreadPool::get_worker_info(true);
    if(!wi) {
      ...
      log_omp.fatal()
          << "OpenMP loop with dynamic scheduling on non-OpenMP Realm processor!";
      abort();
    }
    ...
    log_omp.debug() << "loop dynamic start: start=" << lb << " end=" << ub
                    << " incr=" << st << " chunk=" << chunk;
  ...
  // templated code for __kmpc_dispatch_init_{4,4u,8,8u}
  template <typename T>
  int kmpc_dispatch_next(ident_t *loc, kmp_int32 global_tid, kmp_int32 *p_last, T *p_lb,
                         T *p_ub, typename make_signed<T>::type *p_st)
  {
    Realm::ThreadPool::WorkerInfo *wi = Realm::ThreadPool::get_worker_info(false);
    if(!wi)
      return false; // complained already above
    ...
    int64_t span_start, span_end, stride;
    if(wi->work_item->schedule.next_dynamic(span_start, span_end, stride)) {
      log_omp.debug() << "loop dynamic next: start=" << span_start << " end=" << span_end;
...
REALM_PUBLIC_API
void __kmpc_for_static_init_4(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                              kmp_int32 *plastiter, kmp_int32 *plower, kmp_int32 *pupper,
                              kmp_int32 *pstride, kmp_int32 incr, kmp_int32 chunk)
{
  kmpc_for_static_init<kmp_int32>(loc, global_tid, schedtype, plastiter, plower, pupper,
                                  pstride, incr, chunk);
}

REALM_PUBLIC_API
void __kmpc_for_static_init_4u(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                               kmp_int32 *plastiter, kmp_uint32 *plower,
                               kmp_uint32 *pupper, kmp_uint32 *pstride, kmp_int32 incr,
                               kmp_int32 chunk)
{
  kmpc_for_static_init<kmp_uint32>(loc, global_tid, schedtype, plastiter, plower, pupper,
                                   pstride, incr, chunk);
}

REALM_PUBLIC_API
void __kmpc_for_static_init_8(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                              kmp_int32 *plastiter, kmp_int64 *plower, kmp_int64 *pupper,
                              kmp_int64 *pstride, kmp_int64 incr, kmp_int64 chunk)
{
  kmpc_for_static_init<kmp_int64>(loc, global_tid, schedtype, plastiter, plower, pupper,
                                  pstride, incr, chunk);
}

REALM_PUBLIC_API
void __kmpc_for_static_init_8u(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                               kmp_int32 *plastiter, kmp_uint64 *plower,
                               kmp_uint64 *pupper, kmp_uint64 *pstride, kmp_int64 incr,
                               kmp_int64 chunk)
{
  kmpc_for_static_init<kmp_uint64>(loc, global_tid, schedtype, plastiter, plower, pupper,
                                   pstride, incr, chunk);
}
...
REALM_PUBLIC_API
void __kmpc_dispatch_init_4(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                            kmp_int32 lb, kmp_int32 ub, kmp_int32 st, kmp_int32 chunk)
{
  kmpc_dispatch_init<kmp_int32>(loc, global_tid, schedtype, lb, ub, st, chunk);
}

REALM_PUBLIC_API
void __kmpc_dispatch_init_4u(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                             kmp_uint32 lb, kmp_uint32 ub, kmp_int32 st, kmp_int32 chunk)
{
  kmpc_dispatch_init<kmp_uint32>(loc, global_tid, schedtype, lb, ub, st, chunk);
}

REALM_PUBLIC_API
void __kmpc_dispatch_init_8(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                            kmp_int64 lb, kmp_int64 ub, kmp_int64 st, kmp_int64 chunk)
{
  kmpc_dispatch_init<kmp_int64>(loc, global_tid, schedtype, lb, ub, st, chunk);
}

REALM_PUBLIC_API
void __kmpc_dispatch_init_8u(ident_t *loc, kmp_int32 global_tid, kmp_int32 schedtype,
                             kmp_uint64 lb, kmp_uint64 ub, kmp_int64 st, kmp_int64 chunk)
{
  ...
  kmpc_dispatch_init<kmp_int64>(loc, global_tid, schedtype, lb_signed, ub_signed, st,
                                chunk);
}

REALM_PUBLIC_API
int __kmpc_dispatch_next_4(ident_t *loc, kmp_int32 global_tid, kmp_int32 *p_last,
                           kmp_int32 *p_lb, kmp_int32 *p_ub, kmp_int32 *p_st)
{
  return kmpc_dispatch_next<kmp_int32>(loc, global_tid, p_last, p_lb, p_ub, p_st);
}

REALM_PUBLIC_API
int __kmpc_dispatch_next_4u(ident_t *loc, kmp_int32 global_tid, kmp_int32 *p_last,
                            kmp_uint32 *p_lb, kmp_uint32 *p_ub, kmp_int32 *p_st)
{
  return kmpc_dispatch_next<kmp_uint32>(loc, global_tid, p_last, p_lb, p_ub, p_st);
}

REALM_PUBLIC_API
int __kmpc_dispatch_next_8(ident_t *loc, kmp_int32 global_tid, kmp_int32 *p_last,
                           kmp_int64 *p_lb, kmp_int64 *p_ub, kmp_int64 *p_st)
{
  return kmpc_dispatch_next<kmp_int64>(loc, global_tid, p_last, p_lb, p_ub, p_st);
}

REALM_PUBLIC_API
int __kmpc_dispatch_next_8u(ident_t *loc, kmp_int32 global_tid, kmp_int32 *p_last,
                            kmp_uint64 *p_lb, kmp_uint64 *p_ub, kmp_int64 *p_st)
{
  // map between uint64_t and int64_t by flipping the upper bit
  kmp_int64 lb_signed, ub_signed;
  int ret = kmpc_dispatch_next<kmp_int64>(loc, global_tid, p_last, &lb_signed, &ub_signed,
                                          p_st);
...
kmp_int32 __kmpc_reduce(ident_t *loc, kmp_int32 global_tid, kmp_int32 num_vars,
                        size_t reduce_size, void *reduce_data, kmpc_reduce reduce_func,
...
REALM_PUBLIC_API
kmp_int32 __kmpc_reduce_nowait(ident_t *loc, kmp_int32 global_tid, kmp_int32 nvars,
                               size_t reduce_size, void *reduce_data,
                               kmpc_reduce reduce_func, kmp_critical_name *lck)
{
...
REALM_PUBLIC_API
void __kmpc_critical(ident_t *loc, kmp_int32 global_tid, kmp_critical_name *lck)
{
  Realm::ThreadPool::WorkerInfo *wi = Realm::ThreadPool::get_worker_info(true);
  if(!wi || !wi->work_item || (wi->num_threads == 1)) {
    // already single-threaded, so no further effort needed
...
REALM_PUBLIC_API
void __kmpc_end_critical(ident_t *loc, kmp_int32 global_tid, kmp_critical_name *lck)
{
  Realm::ThreadPool::WorkerInfo *wi = Realm::ThreadPool::get_worker_info(false);
  if(!wi || !wi->work_item || (wi->num_threads == 1)) {
    // already single-threaded, so no further effort needed
