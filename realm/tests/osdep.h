...
static int32_t __sync_fetch_and_add(int32_t *dst, int32_t amt)
{
  return InterlockedAdd((LONG *)dst, amt) - amt;
}
static int32_t __sync_add_and_fetch(int32_t *dst, int32_t amt)
{
  return InterlockedAdd((LONG *)dst, amt);
}
static int32_t __sync_sub_and_fetch(int32_t *dst, int32_t amt)
{
  return InterlockedAdd((LONG *)dst, -amt);
}
static bool __sync_bool_compare_and_swap(volatile uint64_t *tgtptr, uint64_t oldval,
                                         uint64_t newval)
{
...
void accurate_sleep(long long microseconds)
{
  ...
  if(relative > 1.2) {
    std::cerr << "sleep took too long - goal: " << microseconds
              << " us, actual: " << current_time - init_time
              << " us, relative: " << relative << std::endl;
  }
}
