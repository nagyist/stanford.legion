...
{
  return make_cudaExtent(r.hi.x - r.lo.x + 1, r.hi.y - r.lo.y + 1, 0);
}

template <typename T>
static cudaExtent rect_to_extent(Rect<3, T> r)
{
  return make_cudaExtent(r.hi.x - r.lo.x + 1, r.hi.y - r.lo.y + 1, r.hi.z - r.lo.z + 1);
}

template <int N, typename T>
void gpu_task(const void *data, size_t datalen, const void *userdata, size_t userlen,
              Processor p)
{
...
#define USE_EXT_INSTS
  ...
  }
  {
    ...
    InstanceLayoutGeneric *ilg =
        InstanceLayoutGeneric::choose_instance_layout<N, T>(args.extent, ilc, dim_order);
  ...
  CHECK_CUDART(cudaMemcpy2DToArray(
      arr1, 0, 0, reinterpret_cast<const void *>(l0), l1 - l0,
      4 * (args.extent.hi[0] - args.extent.lo[0] + 1),
      args.extent.hi[1] - args.extent.lo[1] + 1, cudaMemcpyHostToDevice));
#endif
  ...
  CHECK_CUDART(cudaMemcpy2DFromArray(reinterpret_cast<void *>(l0), l1 - l0, arr2, 0, 0,
                                     4 * (args.extent.hi[0] - args.extent.lo[0] + 1),
...
template <int N, typename T>
bool test_cuda_arrays(Rect<N, T> extent, Memory src_mem, Processor tgt_proc,
                      Memory tgt_mem)
{
  RegionInstance src_data;
  {
    ...
    if((N > 2) && ((TestConfig::bloat_mask & 4) != 0))
      bloated.hi[2] += 6;
    InstanceLayoutGeneric *layout = InstanceLayoutGeneric::choose_instance_layout(
        IndexSpace<N, T>(bloated), InstanceLayoutConstraints(field_sizes, 0), dim_order);
                    ...
                    size_t userlen, Processor p)
{
  ...
  Memory tgt_mem =
      Machine::MemoryQuery(Machine::get_machine()).best_affinity_to(tgt_proc).first();
  ...
  if(TestConfig::dim_mask & 1) {
    ...
    if(ok)
      ok = test_cuda_arrays(r, src_mem, tgt_proc, tgt_mem);
  }
...
int main(int argc, char **argv)
{
  ...
  Processor::register_task_by_kind(Processor::TOC_PROC, false /*!global*/,
                                   GPU_TASK_BASE + 1, CodeDescriptor(gpu_task<1, int>),
                                   ProfilingRequestSet(), 0, 0)
      .wait();
  Processor::register_task_by_kind(Processor::TOC_PROC, false /*!global*/,
                                   GPU_TASK_BASE + 2, CodeDescriptor(gpu_task<2, int>),
                                   ProfilingRequestSet(), 0, 0)
      .wait();
  Processor::register_task_by_kind(Processor::TOC_PROC, false /*!global*/,
                                   GPU_TASK_BASE + 3, CodeDescriptor(gpu_task<3, int>),
                                   ProfilingRequestSet(), 0, 0)
      .wait();
