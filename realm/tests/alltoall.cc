...
void startup_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                  Processor p)
{
  // Nothing to do here
}

void hookup_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                 Processor p)
{
...
void barrier_task(const void *args, size_t arglen, const void *userdata, size_t userlen,
                  Processor p)
{
...
void measure_all_to_all(Memory::Kind memkind, const char *description, unsigned rank,
                        size_t total_ranks, const std::vector<Processor> &remote_procs)
{
  ...
  for(Machine::MemoryQuery::iterator it = Machine::MemoryQuery(Machine::get_machine())
                                              .only_kind(memkind)
                                              .local_address_space()
                                              .has_capacity(1)
                                              .begin();
  ...
  for(unsigned idx = 0; idx < local_memories.size(); ++idx) {
    RegionInstance::create_instance(local_instances[idx], local_memories[idx], bounds,
                                    field_sizes, 0 /*nop*/, ProfilingRequestSet())
        .wait();
  ...
  if(local_memories.size() > 1) {
    for(unsigned l = 1; l < local_memories.size(); ++l) {
      for(unsigned m = 0; m < local_memories.size(); ++m) {
        ...
        IndexSpace<1, long long> space =
            Rect<1, long long>(lower, lower + TestConfig::size - 1);
        local_events[m2] =
            space.copy(srcs, dsts, ProfilingRequestSet(), local_events[m2]);
      }
      if(TestConfig::full_barrier) {
        std::set<Event> preconditions;
        for(unsigned idx = 0; idx < local_events.size(); ++idx) {
  ...
  for(unsigned r = 1; r < total_ranks; r++) {
    ...
    for(unsigned l = 0; l < local_memories.size(); ++l) {
      ...
      Event done = remote_procs[next_rank].spawn(HOOKUP_TASK_ID, &exchanges.front(),
                                                 exchanges.size() * sizeof(Exchange));
...
int main(int argc, char **argv)
{
  ...
  Event reg0 = Processor::register_task_by_kind(
      Processor::LOC_PROC, false /*global*/, STARTUP_TASK_ID,
      CodeDescriptor(startup_task), ProfilingRequestSet(), 0, 0);
  Event reg1 = Processor::register_task_by_kind(
      Processor::LOC_PROC, false /*global*/, HOOKUP_TASK_ID, CodeDescriptor(hookup_task),
      ProfilingRequestSet(), 0, 0);
  Event reg2 = Processor::register_task_by_kind(
      Processor::LOC_PROC, false /*global*/, BARRIER_TASK_ID,
      CodeDescriptor(barrier_task), ProfilingRequestSet(), 0, 0);
  ...
  rt.collective_spawn_by_kind(Processor::LOC_PROC, STARTUP_TASK_ID, 0, 0,
                              true /*one per node*/,
                              Event::merge_events(reg0, reg1, reg2))
      .wait();
  ...
  measure_all_to_all(Memory::SYSTEM_MEM, "System Memory", rank, total_ranks,
                     remote_procs);
  measure_all_to_all(Memory::REGDMA_MEM, "Registered Memory", rank, total_ranks,
                     remote_procs);
  measure_all_to_all(Memory::GPU_FB_MEM, "Framebuffer Memory", rank, total_ranks,
                     remote_procs);
